-- Migration: attempt_chat_mv exposes the per-attempt feature flags
-- stored directly on attempt_chat_entry.
--
-- Context: grading-op flags (analyses/strengths/improvements) and
-- capability flags (problem_statement/objectives/video/images/questions)
-- are populated per-attempt at creation time (see
-- core/app/tools/entries/attempt_chat/create.py). They were not being
-- projected to the MV, so neither the client UI nor the op-list
-- composition could honor them. This migration surfaces them so the
-- attempt-runtime gates have something to read.
--
-- Drop-and-recreate style, same as 004. Sourced from `c.*`
-- (attempt_chat_entry) — the per-attempt truth. The existing flags
-- that read from `tb.*` (chat_entry template) are left untouched to
-- preserve current behavior for flows that rely on the template
-- defaults.

DROP MATERIALIZED VIEW IF EXISTS public.attempt_chat_mv CASCADE;
...
CREATE UNIQUE INDEX IF NOT EXISTS attempt_chat_mv_chat_id_idx
    ON public.attempt_chat_mv USING btree (chat_id);
