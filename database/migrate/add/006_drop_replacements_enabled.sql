-- Migration: drop `replacements_enabled` column from chat_entry +
-- attempt_chat_entry, and rebuild chat_mv without it.
--
-- Context: `replacements_enabled` was a grading-op flag that never
-- graduated past the scaffold stage — the client has no UI, the
-- generate pipeline never fires a `chat_replacements` op, and the
-- column was silently copied around as dead metadata. Removing it
-- now to prevent drift while the surrounding feature-flag plumbing
-- is being tightened.
--
-- chat_mv depends on tbe.replacements_enabled (SELECT projection);
-- DROP MATERIALIZED VIEW … CASCADE would take anything depending on
-- chat_mv with it, so we drop without CASCADE after recreating the
-- view definition to exclude the column.

DROP MATERIALIZED VIEW IF EXISTS public.chat_mv CASCADE;

ALTER TABLE public.chat_entry DROP COLUMN IF EXISTS replacements_enabled;
ALTER TABLE public.attempt_chat_entry DROP COLUMN IF EXISTS replacements_enabled;
...
CREATE MATERIALIZED VIEW public.chat_mv AS
 WITH scenario_single AS (
         ...
         SELECT ss_1.chat_id,
            COALESCE(bool_or(
                CASE
                    WHEN (fr.type = 'video_enabled'::text) THEN fr.value
                    ELSE NULL::boolean
                END), false) AS video_enabled,
            COALESCE(bool_or(
                CASE
                    WHEN (fr.type = 'problem_statement_enabled'::text) THEN fr.value
                    ELSE NULL::boolean
                END), false) AS problem_statement_enabled,
            COALESCE(bool_or(
                CASE
                    WHEN (fr.type = 'objectives_enabled'::text) THEN fr.value
                    ELSE NULL::boolean
                END), false) AS objectives_enabled,
            COALESCE(bool_or(
                CASE
                    WHEN (fr.type = 'images_enabled'::text) THEN fr.value
                    ELSE NULL::boolean
                END), false) AS images_enabled,
            COALESCE(bool_or(
                CASE
                    WHEN (fr.type = 'questions_enabled'::text) THEN fr.value
                    ELSE NULL::boolean
          ...
          WHERE (fr.type = ANY (ARRAY['video_enabled'::text, 'problem_statement_enabled'::text, 'objectives_enabled'::text, 'images_enabled'::text, 'questions_enabled'::text]))
