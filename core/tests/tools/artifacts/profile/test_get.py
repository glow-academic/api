...
async def test_hides_inactive_by_default(conn):
    created = await create_profile(conn)
    await update_profile(conn, created.id, active=False)
...
async def test_returns_inactive_when_active_filter_is_none(conn):
    ...
    items = await get_profiles(conn, [created.id], active=None)
...
async def test_can_filter_for_only_inactive(conn):
    active_item = await create_profile(conn)
    inactive_item = await create_profile(conn)
    await update_profile(conn, inactive_item.id, active=False)
