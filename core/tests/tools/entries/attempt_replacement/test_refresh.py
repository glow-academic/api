...
async def test_new_attempt_replacement_appears_after_refresh(conn, profile_id):
...
async def test_new_attempt_replacement_is_not_visible_before_refresh(conn, profile_id):
    _created(await _attempt_replacement(conn, profile_id))
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)
