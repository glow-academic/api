...
async def _attempt_replacement(conn, profile_id, **overrides):
    ...
    result = await create_attempt_replacement(conn, **defaults)
    return result
...
async def test_gets_created_attempt_replacement(conn, profile_id):
    _created(await _attempt_replacement(conn, profile_id))
    await refresh_attempt_replacement(conn)
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)
    items = await get_attempt_replacements(conn, ids=[lookup_id])
...
async def test_returns_empty_for_missing_id(conn):
    items = await get_attempt_replacements(conn, ids=[nonexistent_id()])

    assert items == []


async def test_returns_empty_for_empty_ids(conn):
    items = await get_attempt_replacements(conn, ids=[])
