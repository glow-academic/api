...
async def test_gets_created_attempt_highlight(conn, redis_client, profile_id):
...
async def test_returns_empty_for_missing_id(conn, redis_client):
    items = await get_attempt_highlights(conn, ids=[nonexistent_id()], redis=redis_client)

    assert items == []


async def test_returns_empty_for_empty_ids(conn, redis_client):
    items = await get_attempt_highlights(conn, ids=[], redis=redis_client)
