...
async def test_new_attempt_highlight_appears_after_refresh(conn, profile_id):
...
async def test_new_attempt_highlight_is_not_visible_before_refresh(conn, profile_id):
    _created(await _attempt_highlight(conn, profile_id))
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)
