"""Tests for refresh_attempt_highlight."""
...
async def _attempt_highlight(conn, redis_client, profile_id, **overrides):
    ...
    defaults = dict(strength_id=strength.id, session_id=session.id, section="Good opening")
    defaults.update(overrides)
    result = await create_attempt_highlight(conn, redis_client, **defaults)
    return result
...
async def test_new_attempt_highlight_appears_after_refresh(conn, redis_client, profile_id):
    created = _created(await _attempt_highlight(conn, redis_client, profile_id))
    lookup_id = created.id

    await refresh_attempt_highlight(conn)
    items = await get_attempt_highlights(conn, ids=[lookup_id], redis=redis_client)

    assert len(items) >= 1
    assert items[0].highlight_id == lookup_id


async def test_new_attempt_highlight_is_not_visible_before_refresh(conn, redis_client, profile_id):
    created = _created(await _attempt_highlight(conn, redis_client, profile_id))
    lookup_id = created.id
