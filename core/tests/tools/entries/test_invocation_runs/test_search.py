"""Tests for search_test_invocation_runs."""
...
from app.tools.entries.test_invocation_runs.search import search_test_invocation_runs
...
async def test_finds_created_test_invocation_runs(conn, profile_id):
    created = _created(await _test_invocation_runs(conn, profile_id))
    await refresh_test_invocation_runs(conn)
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)
    fetched = await get_test_invocation_runs(conn, ids=[lookup_id])
    row = fetched[0]
    filter_value = getattr(row, 'test_invocation_id', None)
    items = await search_test_invocation_runs(conn, test_invocation_ids=[filter_value], limit_count=20, offset_count=0)
...
async def test_returns_empty_for_unmatched_filter(conn, profile_id):
    created = _created(await _test_invocation_runs(conn, profile_id))
    await refresh_test_invocation_runs(conn)
    items = await search_test_invocation_runs(conn, test_invocation_ids=[nonexistent_id()], limit_count=20, offset_count=0)
...
async def test_respects_limit(conn, profile_id):
    ...
    row = fetched[0]
    filter_value = getattr(row, 'test_invocation_id', None)
    items = await search_test_invocation_runs(conn, test_invocation_ids=[filter_value], limit_count=1, offset_count=0)
