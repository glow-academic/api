"""Tests for refresh_test_invocation_runs."""
...
async def test_new_test_invocation_runs_appears_after_refresh(conn, redis_client, profile_id):
...
async def test_new_test_invocation_runs_is_not_visible_before_refresh(conn, redis_client, profile_id):
    created = _created(await _test_invocation_runs(conn, redis_client, profile_id))
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)

    items = await get_test_invocation_runs(conn, ids=[lookup_id], redis=redis_client, bypass_cache=True)
