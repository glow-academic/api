...
async def test_new_test_invocation_runs_appears_after_refresh(conn, profile_id):
    created = _created(await _test_invocation_runs(conn, profile_id))
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)

    await refresh_test_invocation_runs(conn)
    items = await get_test_invocation_runs(conn, ids=[lookup_id])

    assert len(items) >= 1
    assert items[0].id == lookup_id


async def test_new_test_invocation_runs_is_not_visible_before_refresh(conn, profile_id):
    created = _created(await _test_invocation_runs(conn, profile_id))
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)
