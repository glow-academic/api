...
async def test_gets_created_attempt_practice(conn, profile_id):
    _created(await _attempt_practice(conn, profile_id))
    ...
    items = await get_attempt_practice(conn, attempt_ids=[lookup_id])
...
async def test_returns_empty_for_missing_id(conn):
    items = await get_attempt_practice(conn, attempt_ids=[nonexistent_id()])

    assert items == []


async def test_returns_empty_for_empty_ids(conn):
    items = await get_attempt_practice(conn, attempt_ids=[])
