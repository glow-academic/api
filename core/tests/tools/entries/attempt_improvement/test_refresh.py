...
async def test_new_attempt_improvement_appears_after_refresh(conn, redis_client, profile_id):
    created = _created(await _attempt_improvement(conn, redis_client, profile_id))
    ...
    assert len(items) >= 1
    assert items[0].improvement_id == lookup_id


async def test_new_attempt_improvement_is_not_visible_before_refresh(conn, redis_client, profile_id):
    created = _created(await _attempt_improvement(conn, redis_client, profile_id))
    lookup_id = created.id
