"""Tests for create_attempt_improvement."""
...
async def test_returns_id(conn, profile_id):
    result = await _attempt_improvement(conn, profile_id)
    assert result.id is not None


async def test_visible_via_get_after_refresh(conn, redis_client, profile_id):
    result = await _attempt_improvement(conn, redis_client, profile_id)
    await refresh_attempt_improvement(conn)
    items = await get_attempt_improvements(conn, [result.id], redis_client)
    assert len(items) == 1


async def test_passes_mcp_flag(conn, profile_id):
    result = await _attempt_improvement(conn, profile_id, mcp=True)
    row = await conn.fetchrow(
        "SELECT mcp FROM attempt_improvement_entry WHERE id = $1", result.id
    )
