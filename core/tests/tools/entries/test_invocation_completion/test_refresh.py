"""Tests for refresh_test_invocation_completion."""
...
async def test_new_test_invocation_completion_appears_after_refresh(conn, profile_id):
...
async def test_new_test_invocation_completion_is_not_visible_before_refresh(conn, profile_id):
    _created(await _test_invocation_completion(conn, profile_id))
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)
