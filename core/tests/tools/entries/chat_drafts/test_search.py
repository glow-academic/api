...
async def test_search_finds_created(conn, profile_id):
    ...
    result = await create_chat_draft(conn, session_id=session.id)
...
async def test_search_pagination(conn, profile_id):
    session, group = await _setup(conn, profile_id)
    await create_chat_draft(conn, session_id=session.id)
    await create_chat_draft(conn, session_id=session.id)

    items = await search_chat_drafts(conn, session_ids=[session.id], limit=1)
