...
async def test_finds_created_test_invocation_bridge(conn, redis_client, profile_id):
    ...
    items = await search_test_invocation_bridge(conn, redis_client, test_invocation_ids=[filter_value], limit=20, offset=0)

    assert len(items) >= 1
    assert any(item.test_invocation_id == lookup_id for item in items)
...
async def test_returns_empty_for_unmatched_filter(conn, redis_client, profile_id):
    ...
    await refresh_test_invocation_bridge(conn)
    items = await search_test_invocation_bridge(conn, redis_client, test_invocation_ids=[nonexistent_id()], limit=20, offset=0)
...
async def test_respects_limit(conn, redis_client, profile_id):
    ...
    row = fetched[0]
    filter_value = getattr(row, 'test_invocation_id', None)
    items = await search_test_invocation_bridge(conn, redis_client, test_invocation_ids=[filter_value], limit=1, offset=0)
