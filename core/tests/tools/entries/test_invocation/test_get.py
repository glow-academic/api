...
async def test_gets_created_test_invocation(conn, profile_id):
...
async def test_returns_empty_for_missing_id(conn):
    items = await get_test_invocations(conn, ids=[nonexistent_id()])

    assert items == []


async def test_returns_empty_for_empty_ids(conn):
    items = await get_test_invocations(conn, ids=[])
