"""Tests for refresh_test_invocation."""
...
async def test_new_test_invocation_appears_after_refresh(conn, profile_id):
    _created(await _test_invocation(conn, profile_id))
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)

    await refresh_test_invocation(conn)
    items = await get_test_invocations(conn, ids=[lookup_id])

    assert len(items) >= 1
    assert items[0].id == lookup_id


async def test_new_test_invocation_is_not_visible_before_refresh(conn, profile_id):
    _created(await _test_invocation(conn, profile_id))
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)
