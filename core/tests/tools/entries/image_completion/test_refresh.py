"""Tests for refresh_image_completion."""

import pytest

from app.tools.entries.image_completion.create import create_image_completion
from app.tools.entries.image_completion.refresh import refresh_image_completion
from app.tools.entries.image_completion.refresh import MV_NAME
...
async def _setup_entry(conn, redis_client, profile_id):
    ...
    seed = await create_image(conn, redis_client, session_id=session.id)
...
async def test_refresh_is_idempotent(conn, redis_client, profile_id):
    ...
    result = await create_image_completion(conn, redis_client, image_id=seed.id, session_id=session.id)

    await refresh_image_completion(conn)
...
async def test_refresh_exposes_created_row(conn, redis_client, profile_id):
    session, call, seed = await _setup_entry(conn, redis_client, profile_id)
    result = await create_image_completion(conn, redis_client, image_id=seed.id, session_id=session.id, mcp=True)
    await refresh_image_completion(conn)
