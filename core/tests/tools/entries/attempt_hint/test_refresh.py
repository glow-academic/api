...
async def test_new_attempt_hint_appears_after_refresh(conn, profile_id):
    _created(await _attempt_hint(conn, profile_id))
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)

    await refresh_attempt_hint(conn)
    items = await get_attempt_hints(conn, ids=[lookup_id])

    assert len(items) >= 1
    assert items[0].id == lookup_id


async def test_new_attempt_hint_is_not_visible_before_refresh(conn, profile_id):
    _created(await _attempt_hint(conn, profile_id))
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)
