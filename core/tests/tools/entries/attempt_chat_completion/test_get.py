...
async def test_gets_created_attempt_chat_completion(conn, profile_id):
...
async def test_returns_empty_for_missing_id(conn):
    items = await get_attempt_chat_completions(conn, ids=[nonexistent_id()])

    assert items == []


async def test_returns_empty_for_empty_ids(conn):
    items = await get_attempt_chat_completions(conn, ids=[])
