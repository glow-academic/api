"""Tests for refresh_audio_completion."""

import pytest

from app.tools.entries.audio_completion.create import create_audio_completion
from app.tools.entries.audio_completion.refresh import refresh_audio_completion
from app.tools.entries.audio_completion.refresh import MV_NAME
from app.tools.entries.calls.create import create_call
from app.tools.entries.groups.create import create_group
from app.tools.entries.runs.create import create_run
from app.tools.entries.sessions.create import create_session
from app.tools.entries.audios.create import create_audio

pytestmark = pytest.mark.asyncio


async def _setup_entry(conn, redis_client, profile_id):
    session = await create_session(conn, redis_client, profile_id=profile_id)
    group = await create_group(conn, redis_client, session_id=session.id, artifact_type="persona")
    run = await create_run(conn, redis_client, group_id=group.id, session_id=session.id)
    call = await create_call(conn, redis_client, run_id=run.id, session_id=session.id)
    seed = await create_audio(conn, redis_client, session_id=session.id)
    return session, call, seed


async def test_refresh_is_idempotent(conn, redis_client, profile_id):
    session, call, seed = await _setup_entry(conn, redis_client, profile_id)
    result = await create_audio_completion(conn, redis_client, audio_id=seed.id, session_id=session.id)

    await refresh_audio_completion(conn)

    assert True


async def test_row_not_visible_before_refresh(conn, redis_client, profile_id):
    session, call, seed = await _setup_entry(conn, redis_client, profile_id)
    result = await create_audio_completion(conn, redis_client, audio_id=seed.id, session_id=session.id)

    row = await conn.fetchrow(f"SELECT id FROM {MV_NAME} WHERE id = $1", result.id)

    assert row is None


async def test_refresh_exposes_created_row(conn, redis_client, profile_id):
    session, call, seed = await _setup_entry(conn, redis_client, profile_id)
    result = await create_audio_completion(conn, redis_client, audio_id=seed.id, session_id=session.id, mcp=True)
    await refresh_audio_completion(conn)

    row = await conn.fetchrow(f"SELECT id, mcp FROM {MV_NAME} WHERE id = $1", result.id)

    assert row is not None
    assert row['id'] == result.id
    assert row['mcp'] is True
