...
async def _setup_entry(conn, profile_id):
    ...
    seed = await create_audio(conn, session_id=session.id)
...
async def test_refresh_is_idempotent(conn, profile_id):
    ...
    result = await create_audio_completion(conn, audio_id=seed.id, session_id=session.id)
...
async def test_refresh_exposes_created_row(conn, profile_id):
    session, call, seed = await _setup_entry(conn, profile_id)
    result = await create_audio_completion(conn, audio_id=seed.id, session_id=session.id, mcp=True)
    await refresh_audio_completion(conn)
