...
async def test_gets_created_practice_chat(conn, redis_client, profile_id, simulation_bundle):
...
async def test_returns_empty_for_missing_id(conn, redis_client):
    items = await get_practice_chats(conn, ids=[nonexistent_id()], redis=redis_client)

    assert items == []


async def test_returns_empty_for_empty_ids(conn, redis_client):
    items = await get_practice_chats(conn, ids=[], redis=redis_client)
