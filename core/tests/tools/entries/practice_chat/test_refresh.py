...
async def test_new_practice_chat_appears_after_refresh(conn, redis_client, profile_id, simulation_bundle):
    _, _, _, created = await _practice_chat(conn, redis_client, profile_id, simulation_bundle)
    lookup_id = created.id

    await refresh_practice_chat(conn)
    items = await get_practice_chats(conn, ids=[lookup_id], redis=redis_client)

    assert len(items) >= 1
    assert items[0].id == lookup_id


async def test_new_practice_chat_is_not_visible_before_refresh(conn, redis_client, profile_id, simulation_bundle):
    _, _, _, created = await _practice_chat(conn, redis_client, profile_id, simulation_bundle)
    lookup_id = created.id

    items = await get_practice_chats(conn, ids=[lookup_id], redis=redis_client, bypass_cache=True)
