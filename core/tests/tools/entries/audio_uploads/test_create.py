...
async def test_create_sets_link_fields(conn, redis_client, profile_id):
    """Created row carries the audio/upload/session links and defaults."""
    session, audio, upload = await _deps(conn, redis_client, profile_id)
    result = await create_audio_upload(
        conn, redis_client, audio_id=audio.id, upload_id=upload.id, session_id=session.id
    )
...
async def test_create_cache_matches_db(conn, redis_client, profile_id):
    ...
    cached = await get_audio_upload(conn, result.id, redis_client)
    from_db = await get_audio_upload(conn, result.id, redis_client, bypass_cache=True)

    assert cached is not None
    assert from_db is not None
    assert cached.id == from_db.id
    assert cached.audio_id == from_db.audio_id
...
async def test_soft_create_is_inactive(conn, redis_client, profile_id):
    ...
    row = await get_audio_upload(conn, result.id, redis_client, bypass_cache=True)
...
async def test_honors_explicit_id(conn, redis_client, profile_id):
    ...
    row = await get_audio_upload(conn, explicit_id, redis_client)
    assert row is not None
    assert row.id == explicit_id


async def test_created_entry_is_searchable(conn, redis_client, profile_id):
    """A freshly created active entry is found by search on its audio link."""
    session, audio, upload = await _deps(conn, redis_client, profile_id)
    result = await create_audio_upload(
        conn, redis_client, audio_id=audio.id, upload_id=upload.id, session_id=session.id
    )

    found = await search_audio_uploads(
        conn, redis_client, audio_ids=[audio.id], bypass_cache=True
    )
