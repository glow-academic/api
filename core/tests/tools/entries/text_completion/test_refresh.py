...
async def _setup_entry(conn, profile_id):
    ...
    seed = await create_text(conn, session_id=session.id)
...
async def test_refresh_is_idempotent(conn, profile_id):
    ...
    result = await create_text_completion(conn, text_id=seed.id, session_id=session.id)
...
async def test_refresh_exposes_created_row(conn, profile_id):
    session, call, seed = await _setup_entry(conn, profile_id)
    result = await create_text_completion(conn, text_id=seed.id, session_id=session.id, mcp=True)
    await refresh_text_completion(conn)
