"""Tests for refresh_text_completion."""

import pytest

from app.tools.entries.text_completion.create import create_text_completion
from app.tools.entries.text_completion.refresh import refresh_text_completion
from app.tools.entries.text_completion.refresh import MV_NAME
...
async def _setup_entry(conn, redis_client, profile_id):
    ...
    seed = await create_text(conn, redis_client, session_id=session.id)
...
async def test_refresh_is_idempotent(conn, redis_client, profile_id):
    ...
    result = await create_text_completion(conn, redis_client, text_id=seed.id, session_id=session.id)

    await refresh_text_completion(conn)
...
async def test_refresh_exposes_created_row(conn, redis_client, profile_id):
    session, call, seed = await _setup_entry(conn, redis_client, profile_id)
    result = await create_text_completion(conn, redis_client, text_id=seed.id, session_id=session.id, mcp=True)
    await refresh_text_completion(conn)
