...
async def test_search_finds_created(conn, redis_client, profile_id):
    ...
    result = await create_provider_draft(conn, redis_client, session_id=session.id)

    items = await search_provider_drafts(conn, redis_client, session_ids=[session.id])
...
async def test_search_pagination(conn, redis_client, profile_id):
    session, group = await _setup(conn, redis_client, profile_id)
    await create_provider_draft(conn, redis_client, session_id=session.id)
    await create_provider_draft(conn, redis_client, session_id=session.id)

    items = await search_provider_drafts(conn, redis_client, session_ids=[session.id], limit=1)
