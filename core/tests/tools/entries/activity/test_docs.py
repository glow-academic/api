...
async def test_returns_docs_response(conn):
    result = await get_activity_docs(conn)

    assert result.name == "activity"
...
async def test_includes_materialized_view(conn):
    ...
    assert result.materialized_view.name == "activity_mv"
...
async def test_includes_source_tables(conn):
    ...
    assert "activity_entry" in table_names
    assert "profiles_activity_connection" in table_names
...
async def test_includes_all_operations(conn):
    ...
    assert "create_activity" in op_names
    assert "refresh_activity" in op_names
    assert "get_activity" in op_names
    assert "search_activity" in op_names


async def test_search_operation_has_filters(conn):
    result = await get_activity_docs(conn)

    search_op = next(op for op in result.operations if op.name == "search_activity")
