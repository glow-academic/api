...
async def test_finds_created_activity(conn, redis_client, profile_id):
    ...
    items = await search_activity(conn, redis_client, session_ids=[session.id])
...
async def test_filters_by_session(conn, redis_client, profile_id):
    session = await _session(conn, redis_client, profile_id)
    await create_activity(conn, redis_client, session_id=session.id)
    await refresh_activity(conn)

    items = await search_activity(conn, redis_client, session_ids=[nonexistent_id()])
...
async def test_filters_by_profile(conn, redis_client, profile_id):
    ...
    items = await search_activity(conn, redis_client, profile_ids=[profile_id])
...
async def test_filters_by_date_from(conn, redis_client, profile_id):
    ...
    items = await search_activity(conn, redis_client, date_from=future)
...
async def test_filters_by_date_to(conn, redis_client, profile_id):
    ...
    items = await search_activity(conn, redis_client, date_to=past)
...
async def test_filters_by_mcp(conn, redis_client, profile_id):
    session = await _session(conn, redis_client, profile_id)
    r_mcp = await create_activity(conn, redis_client, session_id=session.id, mcp=True)
    r_normal = await create_activity(conn, redis_client, session_id=session.id, mcp=False)
    await refresh_activity(conn)

    items = await search_activity(conn, redis_client, mcp=True)
...
async def test_pagination_limit(conn, redis_client, profile_id):
    ...
    items = await search_activity(conn, redis_client, session_ids=[session.id], limit=1)
...
async def test_returns_all_without_filter(conn, redis_client, profile_id):
    ...
    items = await search_activity(conn, redis_client)
...
async def test_bypass_mv_finds_without_refresh(conn, redis_client, profile_id):
    ...
    items = await search_activity(conn, redis_client, session_ids=[session.id], bypass_mv=True)
