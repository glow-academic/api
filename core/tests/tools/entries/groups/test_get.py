...
async def test_returns_group_by_id(conn, redis_client, profile_id):
...
async def test_returns_multiple(conn, redis_client, profile_id):
    session = await _session(conn, redis_client, profile_id)
    r1 = await create_group(conn, redis_client, session_id=session.id, artifact_type="persona")
    r2 = await create_group(conn, redis_client, session_id=session.id, artifact_type="persona")
    await refresh_groups(conn)

    items = await get_groups(conn, [r1.id, r2.id], redis_client)
...
async def test_returns_empty_for_missing(conn, redis_client, profile_id):
    items = await get_groups(conn, [nonexistent_id()], redis_client)

    assert items == []


async def test_returns_empty_for_empty_ids(conn, redis_client, profile_id):
    items = await get_groups(conn, [], redis_client)
...
async def test_bypass_mv_returns_without_refresh(conn, redis_client, profile_id):
    ...
    items = await get_groups(conn, [result.id], redis_client, bypass_mv=True)
