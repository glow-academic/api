...
async def test_finds_created_attempt_mutes(conn, redis_client, profile_id):
...
async def test_returns_empty_for_unmatched_filter(conn, redis_client, profile_id):
    created = _created(await _attempt_mutes(conn, redis_client, profile_id))
    await refresh_attempt_mutes(conn)
    items = await search_attempt_mutes_entries_internal(conn, redis_client, conversation_ids=[nonexistent_id()], limit_count=20, offset_count=0)
