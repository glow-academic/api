...
async def test_new_attempt_mutes_appears_after_refresh(conn, redis_client, profile_id):
...
async def test_new_attempt_mutes_is_not_visible_before_refresh(conn, redis_client, profile_id):
    created = _created(await _attempt_mutes(conn, redis_client, profile_id))
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)
