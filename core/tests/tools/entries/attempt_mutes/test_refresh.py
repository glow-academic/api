"""Tests for refresh_attempt_mutes."""
...
async def test_new_attempt_mutes_appears_after_refresh(conn, profile_id):
    created = _created(await _attempt_mutes(conn, profile_id))
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)

    await refresh_attempt_mutes(conn)
    items = await get_attempt_mutes(conn, ids=[lookup_id])

    assert len(items) >= 1
    assert items[0].id == lookup_id


async def test_new_attempt_mutes_is_not_visible_before_refresh(conn, profile_id):
    created = _created(await _attempt_mutes(conn, profile_id))
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)
