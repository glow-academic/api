"""Tests for refresh_attempt_conversation_completion."""
...
from app.tools.entries.attempt_conversation_completion.get import get_attempt_conversation_completions
...
async def test_new_attempt_conversation_completion_appears_after_refresh(conn, profile_id):
...
async def test_new_attempt_conversation_completion_is_not_visible_before_refresh(conn, profile_id):
    _created(await _attempt_conversation_completion(conn, profile_id))
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)
