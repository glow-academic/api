...
async def _setup_entry(conn, redis_client, profile_id):
    ...
    seed = await create_test(conn, redis_client, call_id=call.id)
...
async def test_refresh_is_idempotent(conn, redis_client, profile_id):
    ...
    result = await create_test_completion(conn, redis_client, test_id=seed.id, call_id=call.id)
...
async def test_refresh_exposes_created_row(conn, redis_client, profile_id):
    session, call, seed = await _setup_entry(conn, redis_client, profile_id)
    result = await create_test_completion(conn, redis_client, test_id=seed.id, call_id=call.id, mcp=True)
    await refresh_test_completion(conn)
