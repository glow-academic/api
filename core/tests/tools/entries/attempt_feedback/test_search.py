"""Tests for search_attempt_feedback_entries."""
...
async def test_finds_created_entry(conn, redis_client, profile_id):
    ...
    items = await search_attempt_feedback_entries(conn, redis_client, grade_ids=[grade.id])
...
async def test_filters_by_grade_id(conn, redis_client, profile_id):
    ...
    items = await search_attempt_feedback_entries(conn, redis_client, grade_ids=[nonexistent_id()])

    assert items == []


async def test_pagination_limit(conn, redis_client, profile_id):
    _result, grade = await _setup(conn, redis_client, profile_id)
    await refresh_attempt_feedback(conn)

    items = await search_attempt_feedback_entries(conn, redis_client, grade_ids=[grade.id], limit=1)
...
async def test_returns_all_without_filter(conn, redis_client, profile_id):
    ...
    items = await search_attempt_feedback_entries(conn, redis_client)
...
async def test_bypass_mv_finds_without_refresh(conn, redis_client, profile_id):
    ...
    items = await search_attempt_feedback_entries(
        conn, redis_client, grade_ids=[grade.id], bypass_mv=True
    )
