...
async def test_gets_created_attempt_feedback(conn, redis_client, profile_id):
    created = _created(await _attempt_feedback(conn, redis_client, profile_id))
    await refresh_attempt_feedback(conn)
    lookup_id = created.id
    items = await get_attempt_feedbacks(conn, ids=[lookup_id], redis=redis_client)

    assert len(items) >= 1
    assert items[0].feedback_id == lookup_id


async def test_returns_empty_for_missing_id(conn, redis_client):
    items = await get_attempt_feedbacks(conn, ids=[nonexistent_id()], redis=redis_client)

    assert items == []


async def test_returns_empty_for_empty_ids(conn, redis_client):
    items = await get_attempt_feedbacks(conn, ids=[], redis=redis_client)
