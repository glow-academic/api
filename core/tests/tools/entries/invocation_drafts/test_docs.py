"""Tests for get_invocation_drafts_docs."""
...
async def test_returns_docs_response(conn):
    result = await get_invocation_drafts_docs(conn)

    assert result.name == "invocation_drafts"
...
async def test_includes_materialized_view(conn):
    ...
    assert result.materialized_view.name == "invocation_drafts_mv"
...
async def test_includes_all_operations(conn):
    ...
    assert "create_invocation_draft" in op_names
    assert "refresh_invocation_drafts" in op_names
    assert "get_invocation_drafts" in op_names
    assert "search_invocation_drafts" in op_names
...
async def test_create_operation_has_params(conn):
    ...
    create_op = next(
        op for op in result.operations if op.name == "create_invocation_draft"
    )
...
async def test_search_operation_has_filters(conn):
    ...
    search_op = next(
        op for op in result.operations if op.name == "search_invocation_drafts"
    )
