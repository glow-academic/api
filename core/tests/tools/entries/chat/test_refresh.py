...
async def test_new_chat_appears_after_refresh(conn, redis_client, profile_id, simulation_bundle):
...
async def test_new_chat_is_not_visible_before_refresh(conn, redis_client, profile_id, simulation_bundle):
    _, created = await _chat(conn, redis_client, profile_id, simulation_bundle)
    lookup_id = created.id

    items = await get_chats(conn, ids=[lookup_id], redis=redis_client, bypass_cache=True)
