...
async def test_finds_created_chat(conn, profile_id):
    _created(await _chat(conn, profile_id))
    await refresh_chat(conn)
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)
    fetched = await get_chats(conn, ids=[lookup_id])
    row = fetched[0]
    filter_value = getattr(row, 'parent_id', None)
    items = await search_chat_entries_internal(conn, parent_ids=[filter_value], limit_count=20, offset_count=0)
...
async def test_returns_empty_for_unmatched_filter(conn, profile_id):
    _created(await _chat(conn, profile_id))
    await refresh_chat(conn)
    items = await search_chat_entries_internal(conn, parent_ids=[nonexistent_id()], limit_count=20, offset_count=0)
...
async def test_respects_limit(conn, profile_id):
    ...
    row = fetched[0]
    filter_value = getattr(row, 'parent_id', None)
    items = await search_chat_entries_internal(conn, parent_ids=[filter_value], limit_count=1, offset_count=0)
