...
async def test_new_attempt_grade_appears_after_refresh(conn, profile_id):
    _created(await _attempt_grade(conn, profile_id))
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)

    await refresh_attempt_grade(conn)
    items = await get_attempt_grades(conn, ids=[lookup_id])

    assert len(items) >= 1
    assert items[0].id == lookup_id


async def test_new_attempt_grade_is_not_visible_before_refresh(conn, profile_id):
    _created(await _attempt_grade(conn, profile_id))
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)
