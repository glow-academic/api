"""Tests for get_test_invocation_groups."""
...
async def test_gets_created_test_invocation_groups(conn, profile_id):
    created = _created(await _test_invocation_groups(conn, profile_id))
    await refresh_test_invocation_groups(conn)
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)
    items = await get_test_invocation_groups(conn, ids=[lookup_id])
...
async def test_returns_empty_for_missing_id(conn):
    items = await get_test_invocation_groups(conn, ids=[nonexistent_id()])

    assert items == []


async def test_returns_empty_for_empty_ids(conn):
    items = await get_test_invocation_groups(conn, ids=[])
