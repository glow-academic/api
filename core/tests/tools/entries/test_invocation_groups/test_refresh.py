"""Tests for refresh_test_invocation_groups."""
...
async def test_new_test_invocation_groups_appears_after_refresh(conn, profile_id):
...
async def test_new_test_invocation_groups_is_not_visible_before_refresh(conn, profile_id):
    created = _created(await _test_invocation_groups(conn, profile_id))
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)
