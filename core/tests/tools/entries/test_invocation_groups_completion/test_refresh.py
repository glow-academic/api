"""Tests for refresh_test_invocation_groups_completion."""

import pytest

from app.tools.entries.test_invocation_groups_completion.create import create_test_invocation_groups_completion
from app.tools.entries.test_invocation_groups_completion.refresh import refresh_test_invocation_groups_completion
from app.tools.entries.test_invocation_groups_completion.refresh import MV_NAME
...
async def test_refresh_is_idempotent(conn, profile_id):
    ...
    result = await create_test_invocation_groups_completion(conn, test_invocation_groups_id=seed.id, call_id=call.id)

    await refresh_test_invocation_groups_completion(conn)
...
async def test_refresh_exposes_created_row(conn, profile_id):
    session, call, seed = await _setup_entry(conn, profile_id)
    result = await create_test_invocation_groups_completion(conn, test_invocation_groups_id=seed.id, call_id=call.id, mcp=True)
    await refresh_test_invocation_groups_completion(conn)
