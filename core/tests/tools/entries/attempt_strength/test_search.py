...
async def test_filters_by_message_id(conn, redis_client, profile_id):
    ...
    items = await search_attempt_strengths(conn, redis_client, message_ids=[nonexistent_id()])
...
async def test_filters_by_grade_id_nonexistent(conn, redis_client, profile_id):
    await _setup(conn, redis_client, profile_id)
    await refresh_attempt_strength(conn)

    items = await search_attempt_strengths(conn, redis_client, grade_ids=[nonexistent_id()])
...
async def test_returns_all_without_filter(conn, redis_client, profile_id):
    ...
    items = await search_attempt_strengths(conn, redis_client)
