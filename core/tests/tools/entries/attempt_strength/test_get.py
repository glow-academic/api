"""Tests for get_attempt_strengths."""
...
async def test_gets_created_attempt_strength(conn, redis_client, profile_id):
    created = _created(await _attempt_strength(conn, redis_client, profile_id))
    ...
    assert len(items) >= 1
    assert items[0].strength_id == lookup_id


async def test_returns_empty_for_missing_id(conn, redis_client):
    items = await get_attempt_strengths(conn, ids=[nonexistent_id()], redis=redis_client)

    assert items == []


async def test_returns_empty_for_empty_ids(conn, redis_client):
    items = await get_attempt_strengths(conn, ids=[], redis=redis_client)
