...
async def test_gets_created_attempt_strength(conn, profile_id):
    _created(await _attempt_strength(conn, profile_id))
    await refresh_attempt_strength(conn)
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)
    items = await get_attempt_strengths(conn, ids=[lookup_id])
...
async def test_returns_empty_for_missing_id(conn):
    items = await get_attempt_strengths(conn, ids=[nonexistent_id()])

    assert items == []


async def test_returns_empty_for_empty_ids(conn):
    items = await get_attempt_strengths(conn, ids=[])
