...
async def test_finds_created_entry(conn, redis_client, profile_id):
    ...
    items = await search_attempt_responses(conn, redis_client, chat_ids=[attempt_chat.id])
...
async def test_filters_by_chat_id(conn, redis_client, profile_id):
    ...
    items = await search_attempt_responses(conn, redis_client, chat_ids=[nonexistent_id()])
...
async def test_pagination_limit(conn, redis_client, profile_id):
    ...
    items = await search_attempt_responses(conn, redis_client, chat_ids=[attempt_chat.id], limit=1)
...
async def test_returns_all_without_filter(conn, redis_client, profile_id):
    ...
    items = await search_attempt_responses(conn, redis_client)
