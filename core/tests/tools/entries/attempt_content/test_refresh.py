"""Tests for refresh_attempt_content."""
...
async def _attempt_content(conn, redis_client, profile_id, **overrides):
    ...
    content_persona = await create_persona(conn, redis_client)
    defaults = dict(
    ...
    result = await create_attempt_content(conn, redis_client, **defaults)
    return result
...
async def test_new_attempt_content_appears_after_refresh(conn, redis_client, profile_id):
    created = _created(await _attempt_content(conn, redis_client, profile_id))
    lookup_id = created.id

    await refresh_attempt_content(conn)
    items = await get_attempt_contents(conn, ids=[lookup_id], redis=redis_client)

    assert len(items) >= 1
    assert items[0].content_id == lookup_id


async def test_new_attempt_content_is_not_visible_before_refresh(conn, redis_client, profile_id):
    created = _created(await _attempt_content(conn, redis_client, profile_id))
    lookup_id = created.id
