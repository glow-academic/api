...
async def test_finds_created_entry(conn, redis_client, profile_id):
    ...
    items = await search_test_grades(conn, redis_client, invocation_ids=[test_invocation.id])
...
async def test_filters_by_invocation_id(conn, redis_client, profile_id):
    ...
    items = await search_test_grades(conn, redis_client, invocation_ids=[nonexistent_id()])
...
async def test_pagination_limit(conn, redis_client, profile_id):
    ...
    items = await search_test_grades(conn, redis_client, invocation_ids=[test_invocation.id], limit=1)
...
async def test_returns_all_without_filter(conn, redis_client, profile_id):
    ...
    items = await search_test_grades(conn, redis_client)
