...
async def test_gets_created_images(conn, session_id):
...
async def test_returns_empty_for_missing_id(conn):
    item = await get_image(conn, nonexistent_id())
...
async def test_returns_created_item_after_second_lookup(conn, session_id):
    created = _created(await create_image(conn, session_id=session_id))
    lookup_id = getattr(created, 'image_id', None) or getattr(created, 'id', None) or getattr(created, 'image', None)
    first = await get_image(conn, lookup_id)
    second = await get_image(conn, lookup_id)
