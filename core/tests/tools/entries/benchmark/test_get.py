...
async def test_gets_created_benchmark(conn, profile_id, department_id):
...
async def test_returns_empty_for_missing_id(conn):
    items = await get_benchmarks(conn, ids=[nonexistent_id()])

    assert items == []


async def test_returns_empty_for_empty_ids(conn):
    items = await get_benchmarks(conn, ids=[])
