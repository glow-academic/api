"""Tests for refresh_attempt_message."""
...
async def test_new_attempt_message_appears_after_refresh(conn, profile_id):
...
async def test_new_attempt_message_is_not_visible_before_refresh(conn, profile_id):
    _created(await _attempt_message(conn, profile_id))
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)
