...
async def test_gets_created_attempt_analysis(conn, redis_client, profile_id):
    created = _created(await _attempt_analysis(conn, redis_client, profile_id))
    await refresh_attempt_analysis(conn)
    lookup_id = getattr(created, 'id', None) or getattr(created, 'id', None)
    items = await get_attempt_analyses(conn, redis_client, ids=[lookup_id])
...
async def test_returns_empty_for_missing_id(conn, redis_client):
    items = await get_attempt_analyses(conn, redis_client, ids=[nonexistent_id()])

    assert items == []


async def test_returns_empty_for_empty_ids(conn, redis_client):
    items = await get_attempt_analyses(conn, redis_client, ids=[])
