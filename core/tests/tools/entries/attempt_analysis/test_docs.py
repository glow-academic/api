...
from app.tools.entries.attempt_analysis.docs import get_attempt_analysis_docs
...
async def test_includes_all_operations(conn):
    ...
    assert "search_attempt_analyses" in op_names
...
async def test_create_operation_has_params(conn):
    ...
    create_op = next(
        op for op in result.operations if op.name == "create_attempt_analysis"
    )
