...
async def test_gets_created_test_feedback(conn, redis_client, profile_id):
    created = _created(await _test_feedback(conn, redis_client, profile_id))
...
async def test_returns_empty_for_missing_id(conn, redis_client):
    items = await get_test_feedbacks(conn, ids=[nonexistent_id()], redis=redis_client)

    assert items == []


async def test_returns_empty_for_empty_ids(conn, redis_client):
    items = await get_test_feedbacks(conn, ids=[], redis=redis_client)
