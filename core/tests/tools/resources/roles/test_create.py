...
async def test_creates_new_role(conn, redis_client):
    result = await create_role(
        conn, redis_client, name="New Admin Role", description="desc"
    )
...
async def test_creates_role_with_level(conn, redis_client):
    result = await create_role(
        conn,
        redis_client,
        name="Instructional Role",
        level=2,
    )
...
async def test_visible_via_get(conn, redis_client):
    result = await create_role(conn, redis_client, name="Visible Guest Role")
...
async def test_returns_existing_on_conflict(conn, redis_client):
    first = await create_role(conn, redis_client, name="Dup Role")
    second = await create_role(conn, redis_client, name="Dup Role")

    assert first.id == second.id
    assert second.name == "Dup Role"


async def test_sets_mcp_flag(conn, redis_client):
    result = await create_role(
        conn, redis_client, name="MCP Role", mcp=True
    )
