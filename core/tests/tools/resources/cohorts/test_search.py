...
async def test_respects_limit(conn, redis_client):
    for i in range(5):
        await create_cohort(conn, redis_client, name=f"limit-test-{unique_tag()}")
...
async def test_excludes_ids(conn, redis_client):
    a = await create_cohort(conn, redis_client, name=f"exclude-a-{unique_tag()}")
    b = await create_cohort(conn, redis_client, name=f"exclude-b-{unique_tag()}")
...
async def test_cache_hit(conn, redis_client):
    await create_cohort(conn, redis_client, name=f"cache-hit-{unique_tag()}")
...
async def test_bypass_cache(conn, redis_client):
    await create_cohort(conn, redis_client, name=f"bypass-{unique_tag()}")
