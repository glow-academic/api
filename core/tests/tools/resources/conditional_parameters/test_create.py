...
async def test_creates_new_conditional_parameter(conn, redis_client):
    ...
    result = await create_conditional_parameter(conn, param.parameter_id, redis_client)

    assert result.parameter_id == param.parameter_id
...
async def test_creates_second_row(conn, redis_client):
    ...
    first = await create_conditional_parameter(conn, param1.parameter_id, redis_client)
    second = await create_conditional_parameter(conn, param2.parameter_id, redis_client)
...
async def test_sets_mcp_flag(conn, redis_client):
    ...
    result = await create_conditional_parameter(
        conn, param.parameter_id, redis_client, mcp=True
    )
