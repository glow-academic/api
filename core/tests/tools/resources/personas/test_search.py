...
async def test_finds_created_persona(conn, redis_client):
    await create_persona(conn, redis_client, name="search-test-alpha")

    items = await search_personas(conn, redis_client, search="search-test-alpha")
...
async def test_search_is_case_insensitive(conn, redis_client):
    await create_persona(conn, redis_client, name="CaseTest-Persona")

    items = await search_personas(conn, redis_client, search="casetest-persona")

    assert any(i.name == "CaseTest-Persona" for i in items)
...
async def test_excludes_ids(conn, redis_client):
    a = await create_persona(conn, redis_client, name=f"exclude-a-{unique_tag()}")
...
async def test_returns_empty_for_zero_limit(conn, redis_client):
    items = await search_personas(conn, redis_client, limit_count=0)

    assert items == []


async def test_cache_hit(conn, redis_client):
    await create_persona(conn, redis_client, name=f"cache-hit-{unique_tag()}")

    items1 = await search_personas(conn, redis_client, search="cache-hit-")
    items2 = await search_personas(conn, redis_client, search="cache-hit-")
