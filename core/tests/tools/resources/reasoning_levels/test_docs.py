"""Tests for get_reasoning_levels_docs."""
...
async def test_returns_docs_response(conn):
    ...
    assert result.name == "reasoning_levels"
...
async def test_includes_source_tables(conn):
    ...
    assert "reasoning_levels_resource" in table_names


async def test_includes_all_operations(conn):
    result = await get_reasoning_levels_docs(conn)
    op_names = [op.name for op in result.operations]
    assert "create_reasoning_level" in op_names
