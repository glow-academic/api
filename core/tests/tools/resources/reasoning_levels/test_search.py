...
async def test_finds_created_reasoning_level(conn, redis_client):
    await create_reasoning_level(
        conn, f"search-test-alpha-{unique_tag()}", redis_client
    )

    items = await search_reasoning_levels(
        conn, redis_client, search="search-test-alpha"
    )
...
async def test_search_is_case_insensitive(conn, redis_client):
    await create_reasoning_level(
        conn, f"CaseTest-Reasoning-{unique_tag()}", redis_client
    )

    items = await search_reasoning_levels(
        conn, redis_client, search="casetest-reasoning"
    )
...
async def test_respects_offset(conn, redis_client):
    ...
    all_items = await search_reasoning_levels(
        conn, redis_client, search="offset-test-", limit_count=10
    )
    offset_items = await search_reasoning_levels(
