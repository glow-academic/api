...
def test_get_field_event_config_maps_domain_event_and_entity_scope() -> None:
    ...
    assert config.domain_event_names == ("field.viewed",)
...
def test_drafts_field_event_config_is_collection_scoped() -> None:
    ...
    assert config.domain_event_names == ("field.drafts.viewed",)
...
def test_field_event_types_include_domain_and_lifecycle_events() -> None:
    ...
    assert "field.created" in event_types
    assert "field.updated" in event_types
    assert "field.drafts.viewed" in event_types
