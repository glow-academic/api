"""Tests for provider export — profile check, empty export."""
...
async def test_export_raises_401_for_unknown_profile(monkeypatch):
    async def mock_resolve(pool, pid, redis, **kw):
        return None
    monkeypatch.setattr("app.infra.provider.export.resolve_profile_identity_context", mock_resolve)
    from fastapi import HTTPException
    with pytest.raises(HTTPException) as exc_info:
        await export_provider_impl(_Pool(), None, profile_id=uuid4())
...
async def test_export_returns_empty_for_no_artifacts(monkeypatch):
    ...
    async def mock_enqueue(pool, redis, **kw):
        return None
    monkeypatch.setattr("app.infra.provider.export.resolve_profile_identity_context", mock_resolve)
    monkeypatch.setattr("app.infra.provider.export.search_providers", mock_search)
    monkeypatch.setattr("app.infra.provider.export.create_upload", mock_create_upload)
    monkeypatch.setattr("app.infra.provider.export.create_file_resource", mock_create_file_resource)
    monkeypatch.setattr("app.infra.provider.export.enqueue_refreshes", mock_enqueue)
    result = await export_provider_impl(_Pool(), None, profile_id=uuid4())
