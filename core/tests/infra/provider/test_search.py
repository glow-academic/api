"""Tests for provider search — profile check, empty results."""
...
async def test_search_raises_401_for_unknown_profile(monkeypatch):
    async def mock_resolve(pool, pid, redis, **kw):
        return None
    monkeypatch.setattr("app.infra.provider.search.resolve_profile_identity_context", mock_resolve)
    from fastapi import HTTPException
    with pytest.raises(HTTPException) as exc_info:
        await search_provider_impl(_Pool(), None, profile_id=uuid4())
...
async def test_search_returns_empty_when_no_results(monkeypatch):
    ...
    async def mock_depts(conn, redis, **kw):
        return []
    monkeypatch.setattr("app.infra.provider.search.resolve_profile_identity_context", mock_resolve)
    monkeypatch.setattr("app.infra.provider.search.search_provider_artifacts", mock_search)
    monkeypatch.setattr("app.infra.provider.search.search_departments", mock_depts)
    result = await search_provider_impl(_Pool(), None, profile_id=uuid4())
