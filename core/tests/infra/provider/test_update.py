...
from app.infra.provider.types import UpdateProviderApiRequest, UpdateProviderItem
...
async def test_update_raises_401_for_unknown_profile(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc_info:
        await update_provider_impl(None, None, profile_id=uuid4(), request=UpdateProviderApiRequest(providers=[UpdateProviderItem(id=uuid4())]))
...
async def test_update_raises_400_for_no_items(monkeypatch):
    ...
    from fastapi import HTTPException
    with pytest.raises(HTTPException) as exc_info:
        await update_provider_impl(
            None, None, profile_id=uuid4(),
            request=UpdateProviderApiRequest(providers=[]),
        )
