"""Tests for provider drafts list — profile check, ownership."""
...
async def test_drafts_raises_401_for_unknown_profile(monkeypatch):
    async def mock_resolve(pool, pid, redis, **kw):
        return None
    monkeypatch.setattr("app.infra.provider.drafts.resolve_profile_identity_context", mock_resolve)
    from fastapi import HTTPException
    with pytest.raises(HTTPException) as exc_info:
        await list_provider_drafts_impl(_Pool(), None, profile_id=uuid4())
...
async def test_drafts_returns_context_with_entries(monkeypatch):
    ...
    async def mock_search(conn, redis, *, profile_ids=None, **kwargs):
        return []
    monkeypatch.setattr("app.infra.provider.drafts.resolve_profile_identity_context", mock_resolve)
    monkeypatch.setattr("app.infra.provider.drafts.search_provider_drafts", mock_search)
    result = await list_provider_drafts_impl(_Pool(), None, profile_id=uuid4())
