...
class TestRunCompleteImpl:
    ...
    async def test_contested_run_still_emits_test_proceed_when_resolution_context_store_fails(
        self, conn, redis_client
    ):
        profile, session, _group, run = await _run_graph(conn, redis_client)
        test_id = str(uuid.uuid4())
        ...
        await run_complete_impl(
            ...
            emit=emit,
            conn=conn,
            redis=_RedisSetexFailureProxy(redis_client),
        )

        assert len(events) == 1
        assert events[0].event == "test_proceed"
        assert events[0].data["test_id"] == test_id
        assert await redis_client.get(f"generation_resolution:{test_id}") is None
    ...
    async def test_contested_run_without_generation_test_auto_promotes_and_completes(
        self, conn, redis_client
    ):
        profile, session, _group, run = await _run_graph(conn, redis_client)
        first_name = await create_name(
            conn, "contested-fallback-first", redis_client, soft=True
        )
        second_name = await create_name(
            conn, "contested-fallback-second", redis_client, soft=True
        )
        run_id = str(run.id)
        ...
        assert [event.event for event in events] == ["generation_channel"]
        assert events[0].data["type"] == "complete"
        assert (first_active, second_active) in {(True, False), (False, True)}
        assert await redis_client.exists(f"run:{run_id}:meta") == 0

    async def test_attempt_artifact_emits_grade_complete_when_not_already_emitted(
        self, conn, redis_client
    ):
        profile, session, _group, run = await _run_graph(conn, redis_client)
        soft_name = await create_name(
            conn, "attempt-grade-complete-fresh", redis_client, soft=True
        )
        run_id = str(run.id)
    ...
    async def test_uncontested_run_continues_when_message_persistence_fails(
        self, conn, redis_client, tmp_path
    ):
        profile, session, _group, run = await _run_graph(conn, redis_client)
        soft_name = await create_name(
            conn, "run-complete-save-failure", redis_client, soft=True
        )
        bad_upload_root = tmp_path / "not-a-directory"
        bad_upload_root.write_text("occupied", encoding="utf-8")
        run_id = str(run.id)
    ...
    async def test_chat_artifact_completion_still_returns_when_follow_up_emit_fails(
        self, conn, redis_client
    ):
        profile, session, _group, run = await _run_graph(conn, redis_client)
        soft_name = await create_name(
            conn, "chat-follow-up-failure", redis_client, soft=True
        )
        run_id = str(run.id)
        ...
        async def failing_emit(events_list):
            ...
            call_count += 1
            if call_count == 2:
                raise RuntimeError("post-save emit failed")
        ...
        assert [event.event for event in captured] == ["generation_channel"]

    async def test_attempt_artifact_completion_still_returns_when_grade_emit_fails(
        self, conn, redis_client
    ):
        profile, session, _group, run = await _run_graph(conn, redis_client)
        soft_name = await create_name(
            conn, "attempt-grade-emit-failure", redis_client, soft=True
        )
        run_id = str(run.id)
        ...
        async def failing_emit(events_list):
            ...
            call_count += 1
            if call_count == 2:
                raise RuntimeError("grade emit failed")
            captured.extend(events_list)
