"""Tests for test_run_internal_impl."""
from unittest.mock import AsyncMock
from uuid import uuid4
import pytest
pytestmark = pytest.mark.asyncio

async def test_test_run_internal_impl_requires_profile_id():
    from app.infra.test.run import test_run_internal_impl
    with pytest.raises((ValueError, Exception)):
        await test_run_internal_impl({})

async def test_test_run_internal_impl_is_async():
    from app.infra.test.run import test_run_internal_impl
    import asyncio
    assert asyncio.iscoroutinefunction(test_run_internal_impl)

async def test_test_run_internal_impl_module_uses_audit():
