"""Tests for test_end_internal_impl."""
...
async def test_test_end_internal_impl_requires_profile_id():
    ...
    with pytest.raises((ValueError, Exception)):
        await test_end_internal_impl({})

async def test_test_end_internal_impl_is_async():
    from app.infra.test.end import test_end_internal_impl
    import asyncio
    assert asyncio.iscoroutinefunction(test_end_internal_impl)

async def test_test_end_internal_impl_module_uses_audit():
    import app.infra.test as pkg
    import importlib
    m = importlib.import_module("app.infra.test.end")
