"""Tests for parameter update — monkeypatch collaborators."""
...
from app.infra.parameter.types import UpdateParameterApiRequest, UpdateParameterItem
...
class TestAuth:
    async def test_raises_401_when_profile_not_found(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.infra.parameter.update.resolve_profile_identity_context", fake_resolve,
        )

        with pytest.raises(HTTPException) as exc_info:
            await update_parameter_impl(
                _FakePool(), object(), profile_id=_PROFILE_ID, request=UpdateParameterApiRequest(parameters=[UpdateParameterItem(id=uuid4())]),
            )
...
class TestImport:
    async def test_function_is_importable(self):
        assert callable(update_parameter_impl)
