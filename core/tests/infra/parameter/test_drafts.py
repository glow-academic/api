"""Tests for parameter drafts — monkeypatch collaborators."""
...
class TestAuth:
    async def test_raises_401_when_profile_not_found(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.infra.parameter.drafts.resolve_profile_identity_context", fake_resolve,
        )

        with pytest.raises(HTTPException) as exc_info:
            await list_parameter_drafts_impl(
...
class TestImport:
    async def test_function_is_importable(self):
        assert callable(list_parameter_drafts_impl)
