...
@pytest_asyncio.fixture
async def groups_route_client(
    pool,
    redis_client,
) -> AsyncGenerator[RouteClient, None]:
    """HTTP client mounted on the real groups route stack."""
    import app.infra.globals as globals_mod

    groups_router = _build_artifact_router_for_tests(
...
async def attempt_route_actor(pool, redis_client, setting_graph_factory):
    ...
    return await create_admin_route_actor(
        ...
        # `attempt:export` is likewise required by the export gate in
        # run_artifact_operation_with_audit (attempt is not a default-CRUD
        # artifact, so the helper's default ops don't cover it).
        extra_permissions=[("attempt", "start"), ("attempt", "export")],
