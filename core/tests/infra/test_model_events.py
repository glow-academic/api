...
def test_get_model_event_config_maps_domain_event_and_entity_scope() -> None:
    ...
    assert config.domain_event_names == ("model.viewed",)
...
def test_drafts_model_event_config_is_collection_scoped() -> None:
    ...
    assert config.domain_event_names == ("model.drafts.viewed",)
...
def test_model_event_types_include_domain_and_lifecycle_events() -> None:
    ...
    assert "model.created" in event_types
    assert "model.updated" in event_types
    assert "model.drafts.viewed" in event_types
