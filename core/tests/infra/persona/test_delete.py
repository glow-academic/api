"""Tests for persona delete — monkeypatch collaborators."""
...
class TestAuth:
    async def test_raises_401_when_profile_not_found(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.infra.persona.delete.resolve_profile_identity_context", fake_resolve,
        )
...
class TestImport:
    async def test_function_is_importable(self):
        assert callable(delete_persona_impl)
