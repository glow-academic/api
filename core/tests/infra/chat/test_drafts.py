"""Tests for list_chat_drafts_impl."""
...
async def test_drafts_raises_401_when_no_profile(monkeypatch):
    import app.infra.attempt.chat.drafts as m
    ...
    with pytest.raises((HTTPException, Exception)):
        await m.list_chat_drafts_impl(pool, redis, profile_id=uuid4())

async def test_drafts_function_is_async():
    import app.infra.attempt.chat.drafts as m
    import asyncio
    assert asyncio.iscoroutinefunction(m.list_chat_drafts_impl)
