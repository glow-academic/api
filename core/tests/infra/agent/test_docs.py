"""Tests for agent docs — profile check, response structure."""
...
async def test_docs_raises_401_for_unknown_profile(monkeypatch):
    async def mock_resolve(pool, pid, redis, **kw):
        return None
    monkeypatch.setattr("app.infra.agent.docs.resolve_profile_identity_context", mock_resolve)
    from fastapi import HTTPException
    with pytest.raises(HTTPException) as exc_info:
        await docs_agent_impl(None, None, profile_id=uuid4())
    assert exc_info.value.status_code == 401


async def test_docs_is_async_callable():
    import asyncio
    assert asyncio.iscoroutinefunction(docs_agent_impl)
