"""Tests for agent refresh — MV refresh + cache invalidation."""
...
async def test_refresh_returns_success_with_tags(monkeypatch):
    ...
    monkeypatch.setattr("app.infra.agent.refresh.resolve_profile_identity_context", mock_resolve)
    monkeypatch.setattr("app.infra.agent.refresh.refresh_agent_drafts", mock_refresh_drafts)
    monkeypatch.setattr("app.utils.cache.invalidate_tags.invalidate_tags", mock_invalidate)

    result = await refresh_agent_impl(_FakePool(), "fake-redis", profile_id=uuid4())

    assert result.success is True
    assert "agents" in result.invalidated_tags
    assert "artifacts" in result.invalidated_tags
    assert "agent_drafts_mv" in result.refreshed_views
...
async def test_refresh_raises_401_for_unknown_profile(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc_info:
        await refresh_agent_impl(None, None, profile_id=uuid4())
...
async def test_refresh_skips_cache_when_redis_is_none(monkeypatch):
    ...
    result = await refresh_agent_impl(_FakePool(), None, profile_id=uuid4())
    assert result.success is True
