"""Tests for duplicate_profile_impl."""
...
async def test_duplicate_raises_401_when_no_profile(monkeypatch):
    import app.infra.profile.duplicate as m
    ...
    with pytest.raises((HTTPException, Exception)):
        await m.duplicate_profile_impl(pool, redis, profile_id=uuid4())

async def test_duplicate_function_is_async():
    import app.infra.profile.duplicate as m
    import asyncio
    assert asyncio.iscoroutinefunction(m.duplicate_profile_impl)
