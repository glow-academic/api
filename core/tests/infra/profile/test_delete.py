...
async def test_delete_raises_401_when_no_profile(monkeypatch):
    import app.infra.profile.delete as m
    ...
    with pytest.raises((HTTPException, Exception)):
        await m.delete_profile_impl(pool, redis, profile_id=uuid4())

async def test_delete_function_is_async():
    import app.infra.profile.delete as m
    import asyncio
    assert asyncio.iscoroutinefunction(m.delete_profile_impl)

async def test_delete_module_composes_tools():
    import app.infra.profile.delete as m
