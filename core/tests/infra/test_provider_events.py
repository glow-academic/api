...
def test_get_provider_event_config_maps_domain_event_and_entity_scope() -> None:
    ...
    assert config.domain_event_names == ("provider.viewed",)
...
def test_drafts_provider_event_config_is_collection_scoped() -> None:
    ...
    assert config.domain_event_names == ("provider.drafts.viewed",)
...
def test_provider_event_types_include_domain_and_lifecycle_events() -> None:
    ...
    assert "provider.created" in event_types
    assert "provider.updated" in event_types
    assert "provider.drafts.viewed" in event_types
