"""Tests for document permissions context — monkeypatch DB tool calls."""
...
class TestPermissionsContextNotFound:
    async def test_returns_not_exists_when_artifact_missing(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.infra.document.permissions_context.get_document_artifacts",
            fake_get,
        )

        result = await resolve_document_permissions_context(object(), uuid4())
...
class TestPermissionsContextDataclass:
    async def test_fields_present(self):
        ctx = DocumentPermissionsContext(
