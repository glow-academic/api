"""Tests for document create — monkeypatch collaborators."""
...
class TestAuth:
    async def test_raises_401_when_profile_not_found(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.infra.document.create.resolve_profile_identity_context", fake_resolve,
        )
...
class TestImport:
    async def test_function_is_importable(self):
        assert callable(create_document_impl)
...
from app.infra.document.types import CreateDocumentItem as _AssignItem


@_assign_dataclass
class _AssignDeptScopedProfile:
    """Non-top-level actor (level 1) holding document:create, scoped to one dept."""
...
class TestCreateDepartmentAssignmentGuard:
    ...
    def _patch_common(self, monkeypatch, profile):
        ...
        monkeypatch.setattr(
            "app.infra.document.create.resolve_document_values", fake_values,
        )
    ...
    async def test_cross_department_create_denied(self, monkeypatch):
        ...
        req = CreateDocumentApiRequest(
            documents=[_AssignItem(department_ids=[foreign_dept])]
        )
        with pytest.raises(HTTPException) as exc_info:
            await create_document_impl(
    ...
    async def test_own_department_create_passes_guard(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.infra.document.create.create_denormalized_snapshot", fake_snapshot,
        )

        req = CreateDocumentApiRequest(
            documents=[_AssignItem(department_ids=[own_dept])]
        )
