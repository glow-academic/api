"""Tests for document get — monkeypatch collaborators."""
...
class TestGetAuth:
    async def test_raises_401_when_common_context_returns_none(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.infra.document.get.resolve_common_context",
            fake_resolve,
        )

        with pytest.raises(HTTPException) as exc_info:
            await get_document_impl(
                object(), object(), profile_id=_PROFILE_ID, document_id=None,
            )
...
class TestGetImport:
    async def test_function_is_importable(self):
        assert callable(get_document_impl)
