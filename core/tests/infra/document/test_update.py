"""Tests for document update — monkeypatch collaborators."""
...
from app.infra.document.types import UpdateDocumentApiRequest, UpdateDocumentItem
...
class TestAuth:
    async def test_raises_401_when_profile_not_found(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.infra.document.update.resolve_profile_identity_context", fake_resolve,
        )

        with pytest.raises(HTTPException) as exc_info:
            await update_document_impl(
                _FakePool(), object(), profile_id=_PROFILE_ID, request=UpdateDocumentApiRequest(documents=[UpdateDocumentItem(id=uuid4())]),
            )
...
class TestImport:
    async def test_function_is_importable(self):
        assert callable(update_document_impl)
...
from app.infra.document.types import UpdateDocumentItem as _AssignItem


@_assign_dataclass
class _AssignDeptScopedProfile:
    """Non-top-level actor (level 1) holding document:update, scoped to one dept."""
...
class TestUpdateDepartmentAssignmentGuard:
    ...
    The actor can edit a document in their OWN department but must not be able to
    ...
    def _patch_common(self, monkeypatch, profile, ctx_dept):
        ...
        async def fake_perms(*a, **kw):
            # The document currently lives in ``ctx_dept`` (editable for the actor).
        ...
        monkeypatch.setattr(
            "app.infra.document.update.resolve_document_permissions_context", fake_perms,
        )
        monkeypatch.setattr(
            "app.infra.document.update.resolve_document_values", fake_values,
        )
    ...
    async def test_cross_department_reassignment_denied(self, monkeypatch):
        ...
        # Edit an own-dept document but retag it INTO a foreign dept.
        req = UpdateDocumentApiRequest(
            documents=[_AssignItem(id=uuid4(), department_ids=[foreign_dept])]
        )
        with pytest.raises(HTTPException) as exc_info:
            await update_document_impl(
    ...
    async def test_own_department_reassignment_passes_guard(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.infra.document.update.create_denormalized_snapshot", fake_snapshot,
        )

        # Keep the document in the actor's own dept — guard passes, then we short-circuit
        # at the snapshot step (proving no 403 was raised).
        req = UpdateDocumentApiRequest(
            documents=[_AssignItem(id=uuid4(), department_ids=[own_dept])]
        )
