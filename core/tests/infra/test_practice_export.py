...
async def test_export_raises_401_when_no_profile(monkeypatch):
    import app.infra.practice_export as mod
    ...
    with pytest.raises(HTTPException) as exc:
        await mod.export_practice_client(pool, redis, profile_id=uuid4())
    assert exc.value.status_code == 401

async def test_export_function_is_async():
    import app.infra.practice_export as mod
    import asyncio
    assert asyncio.iscoroutinefunction(mod.export_practice_client)
