"""Tests for scenario refresh — monkeypatch collaborators."""
...
class TestRefreshSuccess:
    async def test_returns_success_with_views_and_tags(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.infra.scenario.refresh.resolve_profile_identity_context",
            fake_resolve,
        )
        monkeypatch.setattr(
            "app.infra.scenario.refresh.refresh_scenario_drafts",
            fake_refresh,
        )
        monkeypatch.setattr(
        ...
        result = await refresh_scenario_impl(_FakePool(), object(), profile_id=_PROFILE_ID)

        assert result.success is True
        assert result.refreshed_views == ["scenario_drafts_mv"]
        assert result.invalidated_tags == ["scenarios", "artifacts"]
...
class TestRefreshAuth:
    async def test_raises_401_when_profile_not_found(self, monkeypatch):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await refresh_scenario_impl(object(), object(), profile_id=_PROFILE_ID)
...
class TestRefreshRedisNone:
    async def test_skips_invalidation_when_redis_is_none(self, monkeypatch):
        ...
        result = await refresh_scenario_impl(_FakePool(), None, profile_id=_PROFILE_ID)
        assert result.success is True
