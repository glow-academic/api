"""Tests for scenario draft — monkeypatch collaborators."""
...
class TestAuth:
    async def test_raises_401_when_profile_not_found(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.infra.scenario.draft.resolve_profile_identity_context", fake_resolve,
        )
...
class TestImport:
    async def test_function_is_importable(self):
        assert callable(patch_scenario_draft_impl)
