...
async def test_can_edit_false_when_agents_active():
    assert compute_can_edit(user_role="admin", active_agent_count=1) is False

async def test_can_edit_true_for_admin_no_agents():
    assert compute_can_edit(user_role="admin", active_agent_count=0) is True

async def test_can_edit_false_for_member():
    assert compute_can_edit(user_role="member", active_agent_count=0) is False

async def test_disabled_reason_active_agents():
    reason = compute_disabled_reason(user_role="admin", active_agent_count=1)
    assert reason is not None
    assert "agents" in reason.lower()

async def test_disabled_reason_none_when_can_edit():
    reason = compute_disabled_reason(user_role="admin", active_agent_count=0)
