"""Tests for docs_tool_impl — docs orchestration."""
...
async def test_docs_raises_401_when_no_profile(monkeypatch):
    import app.infra.tool.docs as mod
    ...
    with pytest.raises(HTTPException) as exc:
        await mod.docs_tool_impl(pool, redis, profile_id=uuid4())
...
async def test_docs_returns_composed_response(monkeypatch):
    ...
    pool.acquire.return_value.__aenter__ = AsyncMock(return_value=AsyncMock())
    pool.acquire.return_value.__aexit__ = AsyncMock(return_value=False)
    result = await mod.docs_tool_impl(pool, redis, profile_id=uuid4())
