...
async def test_create_raises_401_when_no_profile(monkeypatch):
    import app.infra.tool.create as m
    ...
    with pytest.raises((HTTPException, Exception)):
        await m.create_tool_impl(pool, redis, profile_id=uuid4())

async def test_create_function_is_async():
    import app.infra.tool.create as m
    import asyncio
    assert asyncio.iscoroutinefunction(m.create_tool_impl)
