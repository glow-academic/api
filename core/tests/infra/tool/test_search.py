...
async def test_search_raises_401_when_no_profile(monkeypatch):
    import app.infra.tool.search as m
    ...
    with pytest.raises((HTTPException, Exception)):
        await m.search_tool_impl(pool, redis, profile_id=uuid4())

async def test_search_function_is_async():
    import app.infra.tool.search as m
    import asyncio
    assert asyncio.iscoroutinefunction(m.search_tool_impl)
