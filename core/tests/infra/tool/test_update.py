...
async def test_update_raises_401_when_no_profile(monkeypatch):
    import app.infra.tool.update as m
    ...
    with pytest.raises((HTTPException, Exception)):
        await m.update_tool_impl(pool, redis, profile_id=uuid4())

async def test_update_function_is_async():
    import app.infra.tool.update as m
    import asyncio
    assert asyncio.iscoroutinefunction(m.update_tool_impl)
