...
def test_get_simulation_event_config_maps_domain_event_and_entity_scope() -> None:
    ...
    assert config.domain_event_names == ("simulation.viewed",)
...
def test_drafts_simulation_event_config_is_collection_scoped() -> None:
    ...
    assert config.domain_event_names == ("simulation.drafts.viewed",)
...
def test_simulation_event_types_include_domain_and_lifecycle_events() -> None:
    ...
    assert "simulation.created" in event_types
    assert "simulation.updated" in event_types
    assert "simulation.drafts.viewed" in event_types
