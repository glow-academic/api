"""Tests for model get — monkeypatch collaborators."""
...
class TestGetAuth:
    async def test_raises_401_when_common_context_returns_none(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.infra.model.get.resolve_common_context",
            fake_resolve,
        )

        with pytest.raises(HTTPException) as exc_info:
            await get_model_impl(
                object(), object(), profile_id=_PROFILE_ID, model_id=None,
            )
...
class TestGetImport:
    async def test_function_is_importable(self):
        assert callable(get_model_impl)
