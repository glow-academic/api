"""Tests for eval delete — monkeypatch collaborators."""
...
class TestAuth:
    async def test_raises_401_when_profile_not_found(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.infra.eval.delete.resolve_profile_identity_context", fake_resolve,
        )

        with pytest.raises(HTTPException) as exc_info:
            await delete_eval_impl(
...
class TestImport:
    async def test_function_is_importable(self):
        assert callable(delete_eval_impl)
