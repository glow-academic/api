...
async def test_refresh_returns_success(monkeypatch):
    ...
    monkeypatch.setattr("app.infra.group.refresh.resolve_profile_identity_context", AsyncMock(return_value=object()))
...
async def test_refresh_raises_401(monkeypatch):
    monkeypatch.setattr("app.infra.group.refresh.resolve_profile_identity_context", AsyncMock(return_value=None))
