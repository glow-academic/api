"""Tests for patch_rubric_draft_impl."""
...
async def test_draft_raises_401_when_no_profile(monkeypatch):
    import app.infra.rubric.draft as m
    ...
    with pytest.raises((HTTPException, Exception)):
        await m.patch_rubric_draft_impl(pool, redis, profile_id=uuid4())

async def test_draft_function_is_async():
    import app.infra.rubric.draft as m
    import asyncio
    assert asyncio.iscoroutinefunction(m.patch_rubric_draft_impl)
