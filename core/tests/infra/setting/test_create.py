"""Tests for setting create — monkeypatch collaborators."""
...
class TestAuth:
    async def test_raises_401_when_profile_not_found(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.infra.setting.create.resolve_profile_identity_context", fake_resolve,
        )

        with pytest.raises(HTTPException) as exc_info:
            await create_setting_impl(
                _FakePool(), object(), profile_id=_PROFILE_ID,
                request=CreateSettingApiRequest(settings=[]),
            )
...
class TestImport:
    async def test_function_is_importable(self):
        assert callable(create_setting_impl)
