"""Tests for setting docs — monkeypatch collaborators."""
...
class TestAuth:
    async def test_raises_401_when_profile_not_found(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.infra.setting.docs.resolve_profile_identity_context", fake_resolve,
        )

        with pytest.raises(HTTPException) as exc_info:
            await docs_setting_impl(
...
class TestImport:
    async def test_function_is_importable(self):
        assert callable(docs_setting_impl)
