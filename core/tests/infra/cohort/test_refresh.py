"""Tests for cohort refresh — monkeypatch collaborators."""
...
class TestRefreshSuccess:
    async def test_returns_success_with_views_and_tags(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.infra.cohort.refresh.resolve_profile_identity_context",
            fake_resolve,
        )
        monkeypatch.setattr(
            "app.infra.cohort.refresh.refresh_cohort_drafts",
            fake_refresh,
        )
        monkeypatch.setattr(
        ...
        result = await refresh_cohort_impl(_FakePool(), object(), profile_id=_PROFILE_ID)

        assert result.success is True
        assert result.refreshed_views == ["cohort_drafts_mv"]
        assert result.invalidated_tags == ["cohorts", "artifacts"]
...
class TestRefreshAuth:
    async def test_raises_401_when_profile_not_found(self, monkeypatch):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await refresh_cohort_impl(object(), object(), profile_id=_PROFILE_ID)
...
class TestRefreshRedisNone:
    async def test_skips_invalidation_when_redis_is_none(self, monkeypatch):
        ...
        result = await refresh_cohort_impl(_FakePool(), None, profile_id=_PROFILE_ID)
        assert result.success is True
