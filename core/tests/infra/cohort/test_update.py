"""Tests for cohort update — monkeypatch collaborators."""
...
from app.infra.cohort.types import UpdateCohortApiRequest, UpdateCohortItem
...
class TestAuth:
    async def test_raises_401_when_profile_not_found(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.infra.cohort.update.resolve_profile_identity_context", fake_resolve,
        )

        with pytest.raises(HTTPException) as exc_info:
            await update_cohort_impl(
                _FakePool(), object(), profile_id=_PROFILE_ID, request=UpdateCohortApiRequest(cohorts=[UpdateCohortItem(id=uuid4())]),
            )
...
class TestImport:
    async def test_function_is_importable(self):
        assert callable(update_cohort_impl)
...
from app.infra.cohort.types import UpdateCohortItem as _AssignItem


@_assign_dataclass
class _AssignDeptScopedProfile:
    """Non-top-level actor (level 1) holding cohort:update, scoped to one dept."""
...
class TestUpdateDepartmentAssignmentGuard:
    ...
    The actor can edit a cohort in their OWN department but must not be able to
    ...
    def _patch_common(self, monkeypatch, profile, ctx_dept):
        ...
        async def fake_perms(*a, **kw):
            # The cohort currently lives in ``ctx_dept`` (editable for the actor).
        ...
        monkeypatch.setattr(
            "app.infra.cohort.update.resolve_cohort_permissions_context", fake_perms,
        )
        monkeypatch.setattr(
            "app.infra.cohort.update.resolve_cohort_values", fake_values,
        )
    ...
    async def test_cross_department_reassignment_denied(self, monkeypatch):
        ...
        # Edit an own-dept cohort but retag it INTO a foreign dept.
        req = UpdateCohortApiRequest(
            cohorts=[_AssignItem(id=uuid4(), department_ids=[foreign_dept])]
        )
        with pytest.raises(HTTPException) as exc_info:
            await update_cohort_impl(
    ...
    async def test_own_department_reassignment_passes_guard(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.infra.cohort.update.create_denormalized_snapshot", fake_snapshot,
        )

        # Keep the cohort in the actor's own dept — guard passes, then we short-circuit
        # at the snapshot step (proving no 403 was raised).
        req = UpdateCohortApiRequest(
            cohorts=[_AssignItem(id=uuid4(), department_ids=[own_dept])]
        )
