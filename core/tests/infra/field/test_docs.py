"""Tests for field docs — monkeypatch collaborators."""
...
class TestAuth:
    async def test_raises_401_when_profile_not_found(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.infra.field.docs.resolve_profile_identity_context", fake_resolve,
        )

        with pytest.raises(HTTPException) as exc_info:
            await docs_field_impl(
...
class TestImport:
    async def test_function_is_importable(self):
        assert callable(docs_field_impl)
