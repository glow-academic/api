"""Tests for field update — monkeypatch collaborators."""
...
from app.infra.field.types import UpdateFieldApiRequest, UpdateFieldItem
...
class TestAuth:
    async def test_raises_401_when_profile_not_found(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.infra.field.update.resolve_profile_identity_context", fake_resolve,
        )

        with pytest.raises(HTTPException) as exc_info:
            await update_field_impl(
                _FakePool(), object(), profile_id=_PROFILE_ID, request=UpdateFieldApiRequest(fields=[UpdateFieldItem(id=uuid4())]),
            )
...
class TestImport:
    async def test_function_is_importable(self):
        assert callable(update_field_impl)
