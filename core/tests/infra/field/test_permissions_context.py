"""Tests for field permissions context — monkeypatch DB tool calls."""
...
class TestPermissionsContextNotFound:
    async def test_returns_not_exists_when_artifact_missing(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.infra.field.permissions_context.get_field_artifacts",
            fake_get,
        )

        result = await resolve_field_permissions_context(object(), uuid4())
...
class TestPermissionsContextDataclass:
    async def test_fields_present(self):
        ctx = FieldPermissionsContext(
            exists=True, department_ids=[uuid4()], active_parameter_count=0,
        )
        assert ctx.exists is True
        assert len(ctx.department_ids) == 1
        assert ctx.active_parameter_count == 0

    async def test_not_exists_context(self):
        ctx = FieldPermissionsContext(
            exists=False, department_ids=[], active_parameter_count=0,
        )
        assert ctx.exists is False

    async def test_frozen_dataclass(self):
        ctx = FieldPermissionsContext(
            exists=True, department_ids=[], active_parameter_count=0,
        )
