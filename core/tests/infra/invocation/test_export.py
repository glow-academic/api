"""Tests for export_invocation_impl — export orchestration."""
...
async def test_export_raises_401_when_no_profile(monkeypatch):
    import app.infra.invocation.export as mod
    ...
    with pytest.raises(HTTPException) as exc:
        await mod.export_invocation_impl(
            pool, redis, profile_id=uuid4(), test_id=uuid4(), group_id=uuid4()
        )
    assert exc.value.status_code == 401

async def test_export_function_exists():
    import app.infra.invocation.export as mod
    assert callable(mod.export_invocation_impl)
