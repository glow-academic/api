"""Tests for simulation drafts list — profile check, ownership."""
...
async def test_drafts_raises_401_for_unknown_profile(monkeypatch):
    async def mock_resolve(pool, pid, redis, **kw):
        return None
    monkeypatch.setattr("app.infra.simulation.drafts.resolve_profile_identity_context", mock_resolve)
    from fastapi import HTTPException
    with pytest.raises(HTTPException) as exc_info:
        await list_simulation_drafts_impl(_Pool(), None, profile_id=uuid4())
...
async def test_drafts_returns_context_with_entries(monkeypatch):
    ...
    async def mock_search(conn, profile_ids=None):
        return []
    monkeypatch.setattr("app.infra.simulation.drafts.resolve_profile_identity_context", mock_resolve)
    monkeypatch.setattr("app.infra.simulation.drafts.search_simulation_drafts", mock_search)
    result = await list_simulation_drafts_impl(_Pool(), None, profile_id=uuid4())
