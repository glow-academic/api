"""Tests for simulation duplicate — profile check, permission check."""
...
async def test_duplicate_raises_401_for_unknown_profile(monkeypatch):
    async def mock_resolve(pool, pid, redis, **kw):
        return None
    monkeypatch.setattr("app.infra.simulation.duplicate.resolve_profile_identity_context", mock_resolve)
    from fastapi import HTTPException
    with pytest.raises(HTTPException) as exc_info:
        await duplicate_simulation_impl(None, None, profile_id=uuid4(), simulation_id=uuid4())
