"""Tests for simulation get — profile check, 401 handling."""
...
async def test_get_raises_401_for_unknown_profile(monkeypatch):
    async def mock_resolve(*a, **kw):
        return None
    monkeypatch.setattr("app.infra.simulation.get.resolve_common_context", mock_resolve)
    from fastapi import HTTPException
    with pytest.raises(HTTPException) as exc_info:
        await get_simulation_impl(None, None, profile_id=uuid4(), simulation_id=uuid4())
    assert exc_info.value.status_code == 401


async def test_get_function_is_async_callable():
    import asyncio
    assert asyncio.iscoroutinefunction(get_simulation_impl)
