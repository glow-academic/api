"""Tests for simulation draft — profile check, permission check."""
...
async def test_draft_raises_401_for_unknown_profile(monkeypatch):
    async def mock_resolve(pool, pid, redis, **kw):
        return None
    monkeypatch.setattr("app.infra.simulation.draft.resolve_profile_identity_context", mock_resolve)
    from fastapi import HTTPException
    with pytest.raises(HTTPException) as exc_info:
        await patch_simulation_draft_impl(None, None, profile_id=uuid4(), session_id=uuid4(), request=None)
    assert exc_info.value.status_code == 401


async def test_draft_raises_403_for_unauthorized_role(monkeypatch):
