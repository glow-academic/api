"""Auth drafts GET — read from base table + connection tables."""
...
async def get_auth_drafts(
    ...
    *,
    ...
    """Get auth_drafts entries by IDs with connection data.
    ...
    cached_results: dict[str, GetAuthDraftResponse] = {}
    missing_ids: list[UUID] = []
    if not bypass_cache:
        for rid in ids:
            cached = await read_back_row(redis, "auth_drafts", rid)
            if cached is not None and (active is None or cached.get("active") == active):
                cached_results[str(rid)] = GetAuthDraftResponse.model_validate(cached)
            else:
    ...
    mv_results: dict[str, GetAuthDraftResponse] = {}
    for r in rows:
        mv_results[str(r["id"])] = GetAuthDraftResponse(
            id=r["id"],
