...
async def get_eval_drafts(
    ...
    *,
    ...
    cached_results: dict[str, GetEvalDraftResponse] = {}
    missing_ids: list[UUID] = []
    if not bypass_cache:
        for rid in ids:
            cached = await read_back_row(redis, "eval_drafts", rid)
            if cached is not None and (active is None or cached.get("active") == active):
                cached_results[str(rid)] = GetEvalDraftResponse.model_validate(cached)
            else:
    ...
    mv_results: dict[str, GetEvalDraftResponse] = {}
    for r in rows:
        mv_results[str(r["id"])] = GetEvalDraftResponse(
            id=r["id"],
