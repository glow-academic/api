...
async def get_document_drafts(
    ...
    *,
    ...
    cached_results: dict[str, GetDocumentDraftResponse] = {}
    missing_ids: list[UUID] = []
    if not bypass_cache:
        for rid in ids:
            cached = await read_back_row(redis, "document_drafts", rid)
            if cached is not None and (active is None or cached.get("active") == active):
                cached_results[str(rid)] = GetDocumentDraftResponse.model_validate(cached)
            else:
    ...
    mv_results: dict[str, GetDocumentDraftResponse] = {}
    for r in rows:
        mv_results[str(r["id"])] = GetDocumentDraftResponse(
            id=r["id"],
    ...
    out: list[GetDocumentDraftResponse] = []
    for rid in ids:
        key = str(rid)
