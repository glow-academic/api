"""Text completion search — filtered/paginated query against text_completion_mv."""
...
async def search_text_completions(
    ...
    """Search text_completion entries from text_completion_mv with declarative filters."""
    source = await resolve_mv_source(conn, MV_NAME, bypass_mv)

    rows = await conn.fetch(
        f"""
        SELECT id, text_id, stop, error, message, session_id, created_at, active, generated, mcp
        FROM {source}
    ...
    return [GetTextCompletionResponse.model_validate(r) for r in merged]
