...
async def get_field_drafts(
    ...
    *,
    ...
    cached_results: dict[str, GetFieldDraftResponse] = {}
    missing_ids: list[UUID] = []
    if not bypass_cache:
        for rid in ids:
            cached = await read_back_row(redis, "field_drafts", rid)
            if cached is not None and (active is None or cached.get("active") == active):
                cached_results[str(rid)] = GetFieldDraftResponse.model_validate(cached)
            else:
    ...
    mv_results: dict[str, GetFieldDraftResponse] = {}
    for r in rows:
        mv_results[str(r["id"])] = GetFieldDraftResponse(
            id=r["id"],
    ...
    out: list[GetFieldDraftResponse] = []
    for rid in ids:
        key = str(rid)
