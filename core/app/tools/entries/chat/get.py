...
async def get_chats(
    ...
    *,
    ...
    cached_results: dict[str, GetChatResponse] = {}
    missing_ids: list[UUID] = []
    if not bypass_cache:
        for cid in ids:
            cached = await read_back_row(redis, "chat", cid)
            if cached is not None:
                cached_results[str(cid)] = GetChatResponse.model_validate(cached)
            else:
                missing_ids.append(cid)
    else:
        missing_ids = list(ids)

    mv_results: dict[str, GetChatResponse] = {}
    if missing_ids:
        ...
        for r in rows:
            mv_results[str(r["chat_entry_id"])] = GetChatResponse(
                id=r["chat_entry_id"],
