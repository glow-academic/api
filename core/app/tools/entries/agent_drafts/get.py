...
async def get_agent_drafts(
    ...
    *,
    ...
    cached_results: dict[str, GetAgentDraftResponse] = {}
    missing_ids: list[UUID] = []
    if not bypass_cache:
        for rid in ids:
            cached = await read_back_row(redis, "agent_drafts", rid)
            if cached is not None and (active is None or cached.get("active") == active):
                cached_results[str(rid)] = GetAgentDraftResponse.model_validate(cached)
            else:
    ...
    mv_results: dict[str, GetAgentDraftResponse] = {}
    for r in rows:
        mv_results[str(r["id"])] = GetAgentDraftResponse(
            id=r["id"],
    ...
    out: list[GetAgentDraftResponse] = []
    for rid in ids:
        key = str(rid)
