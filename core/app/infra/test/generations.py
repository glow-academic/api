"""Test generations logic — list generation groups for tests.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, permissions)
  2. search_groups — core MV search with artifact_type="test" filter
"""
...
async def generations_test_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """List test generation groups — big-cache wrapped (L3)."""
    return await get_or_build(
        redis=redis,
        key=big_cache_key("test/generations", {
        ...
        builder=lambda: _generations_test_build(
...
async def _generations_test_build(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """List test generation groups.

    Flow:
      1. resolve_profile_identity_context -> role, permissions
      2. Permission check -- test:generations
      3. search_groups with artifact_type="test"
    """
    ...
    if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generations"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to view test generations.",
        )
    ...
    return GenerationsTestApiResponse(
