"""Practice group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating practice groups.
...
class GroupPracticeApiRequest(BaseModel):
    """Request model for practice group endpoint."""
...
class GroupPracticeApiResponse(BaseModel):
    """Response model for practice group endpoint."""
...
async def group_practice_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupPracticeApiRequest | None = None,
    group_id: UUID | None = None,
    name: str | None = None,
    window_seconds: int = DEFAULT_WINDOW_SECONDS,
) -> GroupPracticeApiResponse:
    """Resolve or create a practice group with optional naming.
    ...
    Accepts either a GroupPracticeApiRequest (HTTP/WS) or kwargs (internal).
    """
    ...
    return GroupPracticeApiResponse(
        group_id=resolved_group_id,
