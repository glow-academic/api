"""Field group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating field groups.
...
class GroupFieldApiRequest(BaseModel):
    """Request model for field group endpoint."""
...
class GroupFieldApiResponse(BaseModel):
    """Response model for field group endpoint."""
...
async def group_field_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupFieldApiRequest | None = None,
    ...
    Accepts either a GroupFieldApiRequest (HTTP/WS) or kwargs (internal).
    """
