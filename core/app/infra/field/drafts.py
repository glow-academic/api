"""Field drafts list/search — composable infra architecture.

Composes existing black-box tools — no inline SQL:
  1. resolve_profile_identity_context — caller's profiles_id
  2. search_field_drafts — declarative filters (name ILIKE, date window,
...
Single function powers both:
  ...
  - FE ``POST /field/drafts`` HTTP route

Both consumers receive the same ``GetFieldDraftsApiResponse`` shape; the
...
async def list_field_drafts_impl(
    ...
    **_kwargs,
    ...
    """List/search field drafts owned by the current profile."""
    ...
    return GetFieldDraftsApiResponse(entries=drafts)
