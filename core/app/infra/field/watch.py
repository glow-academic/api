"""Field watch — one-shot block-until-done for the tool layer.

The HTTP route ``GET /field/watch`` stays as live SSE for the browser.
...
convention so ``(field, watch)`` auto-resolves to this callable
...
async def watch_field_impl(
    ...
    **_kwargs,
    ...
    """Watch a field-scoped run in ``group_id``."""
