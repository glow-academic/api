"""Group text download logic — composable infra architecture.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, departments)
  2. has_permission — permission check for group:text_download
...
from app.infra.group.media_types import TextDownloadGroupApiResult
...
async def text_download_group_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    Flow:
      ...
      2. has_permission check (group:text_download)
    ...
    if not has_permission(profile.role_permissions, "system", "text_download"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to download group texts.",
        )
    ...
    return TextDownloadGroupApiResult(
        upload_id=upload.id,
        file_path=file_path,
