"""Group file preview logic — composable infra architecture.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, departments)
  2. has_permission — permission check for group:file_preview
  3. search_file_uploads — resolve file_id -> upload_id
  4. get_upload — resolve upload_id -> file_path, mime_type, size
  5. pdf_first_page_to_image_bytes — generate PNG preview of first page
...
async def file_preview_group_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    Flow:
      ...
      2. has_permission check (group:file_preview)
      ...
      6. pdf_first_page_to_image_bytes -> PNG bytes
    """
    ...
    if not has_permission(profile.role_permissions, "group", "file_preview"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to preview group files.",
        )
    ...
    return FilePreviewGroupApiResult(
