"""Group call download logic — composable infra architecture.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, departments)
  2. has_permission — permission check for group:call_download
...
async def call_download_group_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    Flow:
      ...
      2. has_permission check (group:call_download)
    ...
    if not has_permission(profile.role_permissions, "system", "call_download"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to download group calls.",
        )
    ...
    return CallDownloadGroupApiResult(
        upload_id=upload.id,
        file_path=file_path,
