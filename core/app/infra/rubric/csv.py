"""Rubric CSV parse — accepts raw CSV bytes, auto-maps columns to import fields,
returns mapped items for client-side preview before bulk create.

Routes/rubric/csv.py is a thin HTTP adapter over parse_rubric_csv_impl.
"""
...
async def parse_rubric_csv_impl(
    pool: asyncpg.Pool,
    *,
    session_id: UUID,
    ...
    **_kwargs,
    ...
    """Parse a CSV file and return mapped rubric items for preview.
    ...
    if not field_map:
        labels = ", ".join(f["label"] for f in RUBRIC_IMPORT_FIELDS)
        raise CsvParseError(
    ...
    if soft and call_id is not None:
        async with pool.acquire() as conn:
            await create_soft_call(
                conn, redis, call_id=call_id, artifact="rubric",
