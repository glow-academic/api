...
Composes existing black-box tools — no inline SQL:
  ...
  2. search_rubric_drafts — declarative filters (name ILIKE, date window,
...
Single function powers both:
  ...
  - FE ``POST /rubric/drafts`` HTTP route

Both consumers receive the same ``GetRubricDraftsApiResponse`` shape; the
...
async def list_rubric_drafts_impl(
    ...
    **_kwargs,
    ...
    return GetRubricDraftsApiResponse(entries=drafts)
