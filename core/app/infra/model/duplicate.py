    ...
    *,
    ...
    if accept is not None and idempotency_key is not None:
        ...
        if entry is None or entry.status != "pending" or entry.operation != "duplicate":
            raise HTTPException(
                status_code=404,
                detail="No pending model duplicate for this call.",
            )
        ...
        return DuplicateModelApiResponse(
            success=True,
            model_id=target_id,
            message="Model duplicate accepted" if accept else "Model duplicate rejected",
    ...
    # departments, else a Dept-A actor could clone a Dept-B model
    # they cannot even view (mirrors ``scenario.duplicate``).
    if not compute_can_duplicate(
        role_level=profile.role_level,
        role_permissions=profile.role_permissions,
        model_department_ids=original.department_ids,
