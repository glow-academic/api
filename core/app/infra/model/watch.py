"""Model watch — one-shot block-until-done for the tool layer.

The HTTP route ``GET /model/watch`` stays as live SSE for the browser.
...
convention so ``(model, watch)`` auto-resolves to this callable
...
async def watch_model_impl(
    ...
    **_kwargs,
    ...
    """Watch a model-scoped run in ``group_id``."""
