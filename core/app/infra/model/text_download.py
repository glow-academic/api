"""Model text download logic — composable infra architecture.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, departments)
  2. has_permission — permission check for model:text_download
...
from app.infra.model.types import TextDownloadModelApiResult
...
async def text_download_model_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    if not has_permission(profile.role_permissions, "model", "text_download"):
        raise HTTPException(
            status_code=403,
            detail="You don\'t have permission to download model texts.",
        )
    ...
    return TextDownloadModelApiResult(
        upload_id=upload.id,
        file_path=file_path,
