"""Model generations logic — list generation groups for models.

Composes existing black-box tools:
  1. resolve_profile_identity_context -- profile (role, permissions)
  2. search_groups -- core MV search with artifact_type="model" filter
"""
...
async def generations_model_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """List model generation groups — big-cache wrapped (L3)."""
    return await get_or_build(
        redis=redis,
        key=big_cache_key("model/generations", {
        ...
        response_model=GenerationsModelApiResponse,
        builder=lambda: _generations_model_build(
...
async def _generations_model_build(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """List model generation groups.

    Flow:
      1. resolve_profile_identity_context -> role, permissions
      2. Permission check -- model:generations
      3. search_groups with artifact_type="model"
    """
    ...
    if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generations"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to view model generations.",
        )
    ...
    return GenerationsModelApiResponse(
