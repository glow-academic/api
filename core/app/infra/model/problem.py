"""Model problem logic — per-artifact diagnostic entry point.

Creates a problem entry scoped to the model artifact type.
...
from app.infra.model.types import ProblemModelApiResponse
...
ARTIFACT_TYPE = "model"
PROBLEM_TAGS = ["problems", "views", "models"]
VALID_PROBLEM_TYPES = ("feature", "bug", "question", "other")


async def problem_model_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """Create a problem entry for the model artifact.

    Flow:
      1. Validate type and message
      2. Resolve profile identity context
      3. Permission check -- model:problem
    ...
    if not has_permission(identity.role_permissions, ARTIFACT_TYPE, "problem"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to report model problems.",
        )
    ...
    return ProblemModelApiResponse(
