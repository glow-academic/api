...
def compute_can_delete(
    ...
    if active_agent_count > 0:
        return False
    if not has_permission(role_permissions, "model", "delete"):
        return False

    # Department-subset guard: a non-top-level actor must belong to ALL
    # of the model's departments, else they could delete a model in a
    ...
    if (
        ...
        user_dept_set = {str(d) for d in user_department_ids}
        artifact_dept_set = {str(d) for d in model_department_ids}
...
def compute_can_duplicate(
    role_level: int,
    role_permissions: list[tuple[str, str]],
    model_department_ids: list[str] | list[UUID] | None = None,
    user_department_ids: list[str] | list[UUID] | None = None,
) -> bool:
    """Compute can_duplicate permission.

    Business logic:
    - Must have model:duplicate permission
    - Non-top-level users must belong to ALL of the model's departments
      (mirrors ``scenario.compute_can_duplicate`` — duplicate must not
      bypass the department scope ``has_access`` enforces, else a Dept-A
      user could clone a Dept-B model they cannot even view, inheriting
    ...
    """
    if not has_permission(role_permissions, "model", "duplicate"):
        return False
