...
async def _search_model_build(
    ...
    *,
    ...
    seen: set[UUID] = set()
    for mid in [*model_ids_list, *pending_ledger_ids]:
        if mid in seen:
            continue
        seen.add(mid)
        merged_ids.append(mid)
    added = sum(1 for mid in pending_ledger_ids if mid not in set(model_ids_list))
    ...
    # Mirror the model DETAIL ``get`` ``has_access`` gate on the LIST path so
    # cross-department models the caller could not open in detail are not
