"""Model group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating model groups.
...
class GroupModelApiRequest(BaseModel):
    """Request model for model group endpoint."""
...
class GroupModelApiResponse(BaseModel):
    """Response model for model group endpoint."""
...
async def group_model_impl(
    ...
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupModelApiRequest | None = None,
    ...
    Accepts either a GroupModelApiRequest (HTTP/WS) or kwargs (internal).
    """
