"""Cohort CSV parse — accepts raw CSV bytes, auto-maps columns to import fields,
returns mapped items for client-side preview before bulk create.

Routes/cohort/csv.py is a thin HTTP adapter over parse_cohort_csv_impl.
"""
...
async def parse_cohort_csv_impl(
    pool: asyncpg.Pool,
    *,
    session_id: UUID,
    file_bytes: bytes,
    ...
    **_kwargs,
    ...
    if not field_map:
        labels = ", ".join(f["label"] for f in COHORT_IMPORT_FIELDS)
        raise CsvParseError(
