"""Cohort group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating cohort groups.
...
class GroupCohortApiRequest(BaseModel):
    """Request model for cohort group endpoint."""
...
class GroupCohortApiResponse(BaseModel):
    """Response model for cohort group endpoint."""
...
async def group_cohort_impl(
    ...
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupCohortApiRequest | None = None,
    ...
    Accepts either a GroupCohortApiRequest (HTTP/WS) or kwargs (internal).
    """
