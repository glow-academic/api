"""Cohort call download logic — composable infra architecture.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, departments)
  2. has_permission — permission check for cohort:call_download
...
from app.infra.cohort.types import CallDownloadCohortApiResult
...
async def call_download_cohort_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    with timed("permissions"):
        if not has_permission(profile.role_permissions, "cohort", "call_download"):
            raise HTTPException(
                status_code=403,
                detail="You don\'t have permission to download cohort calls.",
            )
    ...
    return CallDownloadCohortApiResult(
        upload_id=upload.id,
        file_path=file_path,
