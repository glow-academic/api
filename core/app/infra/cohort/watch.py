"""Cohort watch — one-shot block-until-done for the tool layer.

The HTTP route ``GET /cohort/watch`` stays as live SSE for the browser.
...
convention so ``(cohort, watch)`` auto-resolves to this callable
...
async def watch_cohort_impl(
    ...
    **_kwargs,
    ...
    """Watch a cohort-scoped run in ``group_id``."""
