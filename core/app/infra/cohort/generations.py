"""Cohort generations logic — list generation groups for cohorts.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, permissions)
  2. search_groups — core MV search with artifact_type="cohort" filter
"""
...
async def generations_cohort_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """List cohort generation groups — big-cache wrapped (L3)."""
    return await get_or_build(
        redis=redis,
        key=big_cache_key("cohort/generations", {
        ...
        response_model=GenerationsCohortApiResponse,
        builder=lambda: _generations_cohort_build(
...
async def _generations_cohort_build(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """List cohort generation groups.

    Flow:
      1. resolve_profile_identity_context -> role, permissions
      2. Permission check -- cohort:generations
      3. search_groups with artifact_type="cohort"
    """
    ...
    if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generations"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to view cohort generations.",
        )
    ...
    return GenerationsCohortApiResponse(
