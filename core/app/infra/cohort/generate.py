"""Cohort generate logic — deterministic infra architecture.

Calls prepare_generation → execute_generation directly.
Tool calls emit through audit path. Text streaming emits
cohort.generate.progress.
...
async def generate_cohort_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """Cohort generation using deterministic infra functions.
    ...
    with timed("permissions"):
        if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generate"):
            raise HTTPException(
                status_code=403,
                detail="You don't have permission to generate cohorts.",
            )

    # RL-A: Rate limit (RL-A / RL1/RL2) — every LLM-generation entry point
    # must meter the request_limit. #379 only metered attempt/test/system
    # (+agent), but cohort/generate is an LLM-cost entry point too; enforce
    # here before any provider call. No-op for roles without a quota.
    ...
    try:
        ...
        logger.info(
            f"GENERATE_COHORT: prepared run_id={prepared.run_id}, "
        ...
        with timed("execute"):
            run_result = await run_generation_with_refresh(
                ...
                tool_soft=tool_soft,
                artifact_type=ARTIFACT_TYPE,
                refresh_fn=refresh_cohort_impl,
                profile_id=profile_id,
                session_id=session_id,
                group_id=group_id,
        ...
        logger.exception(f"Cohort generation failed: {e}")
        raise
