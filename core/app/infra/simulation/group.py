"""Simulation group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating simulation groups.
...
class GroupSimulationApiRequest(BaseModel):
    """Request model for simulation group endpoint."""
...
class GroupSimulationApiResponse(BaseModel):
    """Response model for simulation group endpoint."""
...
async def group_simulation_impl(
    ...
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupSimulationApiRequest | None = None,
    ...
    Accepts either a GroupSimulationApiRequest (HTTP/WS) or kwargs (internal).
    """
    ...
    return GroupSimulationApiResponse(
        group_id=resolved_group_id,
