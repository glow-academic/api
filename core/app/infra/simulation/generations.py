"""Simulation generations logic — list generation groups for simulations.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, permissions)
  2. search_groups — core MV search with artifact_type="simulation" filter
"""
...
async def generations_simulation_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """List simulation generation groups — big-cache wrapped (L3)."""
    return await get_or_build(
        redis=redis,
        key=big_cache_key("simulation/generations", {
        ...
        tags=["context", "simulation"],
        ttl_s=DEFAULT_BIG_CACHE_TTL_S,
        response_model=GenerationsSimulationApiResponse,
        builder=lambda: _generations_simulation_build(
...
async def _generations_simulation_build(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """List simulation generation groups.

    Flow:
      1. resolve_profile_identity_context -> role, permissions
      2. Permission check — simulation:generations
      3. search_groups with artifact_type="simulation"
    """
    ...
    if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generations"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to view simulation generations.",
        )
    ...
    return GenerationsSimulationApiResponse(
