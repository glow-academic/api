"""Simulation call download logic — composable infra architecture.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, departments)
  2. has_permission — permission check for simulation:call_download
...
from app.infra.simulation.types import CallDownloadSimulationApiResult
...
async def call_download_simulation_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    if not has_permission(profile.role_permissions, "simulation", "call_download"):
        raise HTTPException(
            status_code=403,
            detail="You don\'t have permission to download simulation calls.",
        )
    ...
    return CallDownloadSimulationApiResult(
        upload_id=upload.id,
        file_path=file_path,
