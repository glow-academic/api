"""Simulation watch — one-shot block-until-done for the tool layer.

The HTTP route ``GET /simulation/watch`` stays as live SSE for the browser.
...
convention so ``(simulation, watch)`` auto-resolves to this callable
...
async def watch_simulation_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """Watch a simulation-scoped run in ``group_id``."""
