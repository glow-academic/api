"""Department drafts list/search — composable infra architecture.

Composes existing black-box tools — no inline SQL:
  1. resolve_profile_identity_context — caller's profiles_id
  2. search_department_drafts — declarative filters (name ILIKE, date window,
...
Single function powers both:
  ...
  - FE ``POST /department/drafts`` HTTP route

Both consumers receive the same ``GetDepartmentDraftsApiResponse`` shape; the
...
from app.tools.entries.department_drafts.search import search_department_drafts
...
async def list_department_drafts_impl(
    ...
    **_kwargs,
    ...
    """List/search department drafts owned by the current profile."""
    ...
    return GetDepartmentDraftsApiResponse(entries=drafts)
