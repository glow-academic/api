"""Department generations logic — list generation groups for departments.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, permissions)
  2. search_groups — core MV search with artifact_type="department" filter
"""
...
async def generations_department_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """List department generation groups — big-cache wrapped (L3)."""
    return await get_or_build(
        redis=redis,
        key=big_cache_key("department/generations", {
        ...
        tags=["context", "department"],
        ttl_s=DEFAULT_BIG_CACHE_TTL_S,
        response_model=GenerationsDepartmentApiResponse,
        builder=lambda: _generations_department_build(
...
async def _generations_department_build(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """List department generation groups.

    Flow:
      1. resolve_profile_identity_context → role, permissions
      2. Permission check — department:generations
      3. search_groups with artifact_type="department"
    """
    ...
    if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generations"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to view department generations.",
        )
    ...
    return GenerationsDepartmentApiResponse(
