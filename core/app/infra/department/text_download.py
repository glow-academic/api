"""Department text download logic — composable infra architecture.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, departments)
  2. has_permission — permission check for department:text_download
...
from app.infra.department.types import TextDownloadDepartmentApiResult
...
async def text_download_department_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    with timed("permissions"):
        if not has_permission(profile.role_permissions, "department", "text_download"):
            raise HTTPException(
                status_code=403,
                detail="You don\'t have permission to download department texts.",
            )
    ...
    return TextDownloadDepartmentApiResult(
        upload_id=upload.id,
        file_path=file_path,
