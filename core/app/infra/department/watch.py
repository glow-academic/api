"""Department watch — one-shot block-until-done for the tool layer.

The HTTP route ``GET /department/watch`` stays as live SSE for the browser.
...
convention so ``(department, watch)`` auto-resolves to this callable
...
async def watch_department_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """Watch a department-scoped run in ``group_id``."""
