"""Department generate logic — deterministic infra architecture."""
...
async def generate_department_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """Department generation using deterministic infra functions."""
    ...
    with timed("permissions"):
        if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generate"):
            raise HTTPException(
                status_code=403,
                detail="You don't have permission to generate departments.",
            )
    ...
    try:
      with timed("prepare"):
        ...
        logger.info(
            f"GENERATE_DEPARTMENT: prepared run_id={prepared.run_id}, "
        ...
        with timed("execute"):
            run_result = await run_generation_with_refresh(
                ...
                tool_soft=tool_soft,
                artifact_type=ARTIFACT_TYPE,
                refresh_fn=refresh_department_impl,
                profile_id=profile_id,
                session_id=session_id,
                group_id=group_id,
        ...
        logger.exception(f"Department generation failed: {exc}")
        raise
