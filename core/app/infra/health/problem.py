"""Health problem logic — per-artifact diagnostic entry point.

Creates a problem entry scoped to the health artifact type.
...
from app.infra.health.types import ProblemHealthApiResponse
...
PROBLEM_TAGS = ["problems", "views", "health"]
VALID_PROBLEM_TYPES = ("feature", "bug", "question", "other")


async def problem_health_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """Create a problem entry for the health artifact.

    Flow:
      1. Validate type and message
      2. Resolve profile identity context
      3. Permission check — health:problem
    ...
    if not has_permission(identity.role_permissions, ARTIFACT_TYPE, "problem"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to report health problems.",
        )
    ...
    return ProblemHealthApiResponse(
