"""Health group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating health groups.
...
class GroupHealthApiRequest(BaseModel):
    """Request model for health group endpoint."""
...
class GroupHealthApiResponse(BaseModel):
    """Response model for health group endpoint."""
...
async def group_health_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupHealthApiRequest | None = None,
    group_id: UUID | None = None,
    name: str | None = None,
    window_seconds: int = DEFAULT_WINDOW_SECONDS,
) -> GroupHealthApiResponse:
    """Resolve or create a health group with optional naming.
    ...
    Accepts either a GroupHealthApiRequest (HTTP/WS) or kwargs (internal).
    """
