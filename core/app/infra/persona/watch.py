"""Persona watch — one-shot block-until-done for the tool layer.

The HTTP route ``GET /persona/watch`` stays as live SSE for the browser.
...
convention so ``(persona, watch)`` auto-resolves to this callable
...
async def watch_persona_impl(
    ...
    **_kwargs,
    ...
    """Watch a persona-scoped run in ``group_id``."""
