"""Persona text download logic — composable infra architecture.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, departments)
  2. has_permission — permission check for persona:text_download
...
from app.infra.persona.types import TextDownloadPersonaApiResult
from app.infra.profile_identity_context import resolve_profile_identity_context
from app.tools.entries.text_uploads.search import search_text_uploads
from app.tools.entries.uploads.get import get_upload


async def text_download_persona_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    if not has_permission(profile.role_permissions, "persona", "text_download"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to download persona texts.",
        )
    ...
    return TextDownloadPersonaApiResult(
        upload_id=upload.id,
        file_path=file_path,
