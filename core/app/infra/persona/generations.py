"""Persona generations logic — list generation groups for personas.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, permissions)
  2. search_groups — core MV search with artifact_type="persona" filter
"""
...
async def generations_persona_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """List persona generation groups — big-cache wrapped (L3)."""
    return await get_or_build(
        redis=redis,
        key=big_cache_key("persona/generations", {
        ...
        builder=lambda: _generations_persona_build(
...
async def _generations_persona_build(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    Flow:
      ...
      2. Permission check — persona:generations
      3. search_groups with artifact_type="persona"
    """
    ...
    with timed("permissions"):
        if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generations"):
            raise HTTPException(
                status_code=403,
                detail="You don't have permission to view persona generations.",
            )
    ...
    return GenerationsPersonaApiResponse(
