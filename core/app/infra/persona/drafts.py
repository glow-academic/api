"""Persona drafts list/search — composable infra architecture.

Composes existing black-box tools — no inline SQL:
  1. resolve_profile_identity_context — caller's profiles_id
  2. search_persona_drafts — declarative filters (name ILIKE, date window,
...
Single function powers both:
  ...
  - FE ``POST /persona/drafts`` HTTP route

Both consumers receive the same ``GetPersonaDraftsApiResponse`` shape; the
