...
async def generate_parameter_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """Parameter generation using deterministic infra functions.
    ...
    with timed("permissions"):
        if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generate"):
            raise HTTPException(
                status_code=403,
                detail="You don't have permission to generate parameters.",
            )
    ...
    try:
        ...
        logger.info(
            f"GENERATE_PARAMETER: prepared run_id={prepared.run_id}, "
        ...
        logger.exception(f"Parameter generation failed: {e}")
        raise
