"""Parameter drafts list/search — composable infra architecture.

Composes existing black-box tools — no inline SQL:
  1. resolve_profile_identity_context — caller's profiles_id
  2. search_parameter_drafts — declarative filters (name ILIKE, date window,
...
Single function powers both:
  ...
  - FE ``POST /parameter/drafts`` HTTP route

Both consumers receive the same ``GetParameterDraftsApiResponse`` shape; the
...
from app.tools.entries.parameter_drafts.search import search_parameter_drafts


async def list_parameter_drafts_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """List/search parameter drafts owned by the current profile."""
    ...
    async with pool.acquire() as conn:
        drafts = await search_parameter_drafts(
            conn,
    ...
    return GetParameterDraftsApiResponse(entries=drafts)
