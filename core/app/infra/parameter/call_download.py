"""Parameter call download logic — composable infra architecture.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, departments)
  2. has_permission — permission check for parameter:call_download
...
from app.infra.parameter.types import CallDownloadParameterApiResult
...
async def call_download_parameter_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    with timed("permissions"):
        if not has_permission(profile.role_permissions, "parameter", "call_download"):
            raise HTTPException(
                status_code=403,
                detail="You don\'t have permission to download parameter calls.",
            )
    ...
    return CallDownloadParameterApiResult(
        upload_id=upload.id,
        file_path=file_path,
