"""Parameter refresh logic — composable infra architecture.

Canonical refresh function for parameter MVs. Composes black-box entry tools:
...
# Black-box entry refresh tools — parameter-specific
from app.tools.entries.parameter_drafts.refresh import refresh_parameter_drafts
...
_REFRESH_FNS = {
    "parameter_drafts_mv": refresh_parameter_drafts,
...
async def refresh_parameter_impl(
    ...
    **_kwargs,
    ...
    """Parameter refresh using composable infra functions.

    Accepts either a RefreshParameterApiRequest (HTTP/WS) or kwargs (internal).
