"""Parameter group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating parameter groups.
...
class GroupParameterApiRequest(BaseModel):
    """Request model for parameter group endpoint."""
...
class GroupParameterApiResponse(BaseModel):
    """Response model for parameter group endpoint."""
...
async def group_parameter_impl(
    ...
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupParameterApiRequest | None = None,
    ...
    Accepts either a GroupParameterApiRequest (HTTP/WS) or kwargs (internal).
    """
