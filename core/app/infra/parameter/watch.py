"""Parameter watch — one-shot block-until-done for the tool layer.

The HTTP route ``GET /parameter/watch`` stays as live SSE for the browser.
...
convention so ``(parameter, watch)`` auto-resolves to this callable
...
async def watch_parameter_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """Watch a parameter-scoped run in ``group_id``."""
