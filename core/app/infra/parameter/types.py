...
class CreateParameterApiRequest(BaseModel):
    ...
    parameters: list[CreateParameterItem] = Field(..., max_length=MAX_BULK_ITEMS, description="List of parameters to create")
...
class FileDownloadParameterApiRequest(BaseModel):
    """Request model for parameter file download endpoint."""

    file_id: UUID = Field(..., description="UUID of the files_resource to download")


class FileDownloadParameterApiResult(BaseModel):
