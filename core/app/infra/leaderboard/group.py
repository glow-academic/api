"""Leaderboard group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating leaderboard groups.
...
class GroupLeaderboardApiRequest(BaseModel):
    """Request model for leaderboard group endpoint."""
...
class GroupLeaderboardApiResponse(BaseModel):
    """Response model for leaderboard group endpoint."""
...
async def group_leaderboard_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupLeaderboardApiRequest | None = None,
    ...
    Accepts either a GroupLeaderboardApiRequest (HTTP/WS) or kwargs (internal).
    """
    ...
    return GroupLeaderboardApiResponse(
        group_id=resolved_group_id,
