"""Eval file download logic — composable infra architecture.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, departments)
  2. has_permission — permission check for eval:file_download
...
from app.infra.eval.types import FileDownloadEvalApiResult
...
async def file_download_eval_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    Flow:
      ...
      2. has_permission check (eval:file_download)
    ...
    with timed("permissions"):
        if not has_permission(profile.role_permissions, "eval", "file_download"):
            raise HTTPException(
                status_code=403,
                detail="You don't have permission to download eval files.",
            )
    ...
    return FileDownloadEvalApiResult(
