"""Eval call download logic — composable infra architecture.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, departments)
  2. has_permission — permission check for eval:call_download
...
from app.infra.eval.types import CallDownloadEvalApiResult
...
async def call_download_eval_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    if not has_permission(profile.role_permissions, "eval", "call_download"):
        raise HTTPException(
            status_code=403,
            detail="You don\'t have permission to download eval calls.",
        )
    ...
    return CallDownloadEvalApiResult(
        upload_id=upload.id,
        file_path=file_path,
