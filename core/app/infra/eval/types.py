...
class CreateEvalApiRequest(BaseModel):
    ...
    evals: list[CreateEvalItem] = Field(..., max_length=MAX_BULK_ITEMS, description="List of evals to create")
...
class FileDownloadEvalApiRequest(BaseModel):
    """Request model for eval file download endpoint."""

    file_id: UUID = Field(..., description="UUID of the files_resource to download")


class FileDownloadEvalApiResult(BaseModel):
