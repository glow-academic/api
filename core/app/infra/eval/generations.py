"""Eval generations logic — list generation groups for evals.

Composes existing black-box tools:
  1. resolve_profile_identity_context -- profile (role, permissions)
  2. search_groups -- core MV search with artifact_type="eval" filter
"""
...
async def generations_eval_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """List eval generation groups — big-cache wrapped (L3)."""
    return await get_or_build(
        redis=redis,
        key=big_cache_key("eval/generations", {
        ...
        builder=lambda: _generations_eval_build(
...
async def _generations_eval_build(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """List eval generation groups.

    Flow:
      1. resolve_profile_identity_context -> role, permissions
      2. Permission check -- eval:generations
      3. search_groups with artifact_type="eval"
    """
    ...
    if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generations"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to view eval generations.",
        )
    ...
    return GenerationsEvalApiResponse(
