"""Eval watch — one-shot block-until-done for the tool layer.

The HTTP route ``GET /eval/watch`` stays as live SSE for the browser.
...
convention so ``(eval, watch)`` auto-resolves to this callable
...
async def watch_eval_impl(
    ...
    **_kwargs,
    ...
    """Watch a eval-scoped run in ``group_id``."""
