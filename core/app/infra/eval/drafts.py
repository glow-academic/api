"""Eval drafts list/search — composable infra architecture.

Composes existing black-box tools — no inline SQL:
  1. resolve_profile_identity_context — caller's profiles_id
  2. search_eval_drafts — declarative filters (name ILIKE, date window,
...
Single function powers both:
  ...
  - FE ``POST /eval/drafts`` HTTP route

Both consumers receive the same ``GetEvalDraftsApiResponse`` shape; the
...
async def list_eval_drafts_impl(
    ...
    **_kwargs,
    ...
    """List/search eval drafts owned by the current profile."""
    ...
    return GetEvalDraftsApiResponse(entries=drafts)
