"""Eval generate logic — deterministic infra architecture."""
...
async def generate_eval_impl(
    ...
    **_kwargs,
    ...
    """Eval generation using deterministic infra functions."""
    ...
    with timed("permissions"):
        if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generate"):
            raise HTTPException(
                status_code=403,
                detail="You don't have permission to generate evals.",
            )
    ...
    try:
        ...
        logger.info(
            f"GENERATE_EVAL: prepared run_id={prepared.run_id}, "
        ...
        logger.exception(f"Eval generation failed: {exc}")
        raise
