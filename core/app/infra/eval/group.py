"""Eval group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating eval groups.
...
class GroupEvalApiRequest(BaseModel):
    """Request model for eval group endpoint."""
...
class GroupEvalApiResponse(BaseModel):
    """Response model for eval group endpoint."""
...
async def group_eval_impl(
    ...
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupEvalApiRequest | None = None,
    ...
    Accepts either a GroupEvalApiRequest (HTTP/WS) or kwargs (internal).
    """
