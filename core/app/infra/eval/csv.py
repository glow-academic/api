"""Eval CSV parse — accepts raw CSV bytes, auto-maps columns to import fields,
returns mapped items for client-side preview before bulk create.

Routes/eval/csv.py is a thin HTTP adapter over parse_eval_csv_impl.
"""
...
class ParseEvalCsvApiResponse(BaseModel):
    """Response for CSV parse — mapped items ready for review."""

    upload_id: UUID
    items: list[CreateEvalItem]
...
def _build_field_map(headers: list[str]) -> dict[int, dict[str, Any]]:
    ...
    for field in EVAL_IMPORT_FIELDS:
...
def _row_to_item(row: list[str], field_map: dict[int, dict[str, Any]]) -> CreateEvalItem:
    kwargs: dict[str, Any] = {}
    for col_idx, field_def in field_map.items():
        ...
        if field_type == "boolean":
            ...
            kwargs[key] = raw
    return CreateEvalItem(**kwargs)


async def parse_eval_csv_impl(
    pool: asyncpg.Pool,
    *,
    session_id: UUID,
    ...
    **_kwargs,
    ...
    """Parse a CSV file and return mapped eval items for preview.
    ...
    if accept is not None and idempotency_key is not None:
        async with pool.acquire() as conn:
            entry = await get_soft_call(conn, idempotency_key, redis, artifact="eval")
        ...
        async with pool.acquire() as conn:
            await create_soft_call(
                conn, redis, call_id=idempotency_key, artifact="eval",
                operation="csv", artifact_id=entry.artifact_id,
                status="accepted" if accept else "rejected",
            )
        return ParseEvalCsvApiResponse(
    ...
    with timed("map_fields"):
        ...
        if not field_map:
            labels = ", ".join(f["label"] for f in EVAL_IMPORT_FIELDS)
            raise CsvParseError(
        ...
        required_keys = {f["key"] for f in EVAL_IMPORT_FIELDS if f.get("required")}
    ...
    if soft and call_id is not None:
        async with pool.acquire() as conn:
            await create_soft_call(
                conn, redis, call_id=call_id, artifact="eval",
