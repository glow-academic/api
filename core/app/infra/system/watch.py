"""System watch — one-shot block-until-done for the tool layer.

The HTTP route ``GET /system/watch`` stays as live SSE for the browser.
...
convention so ``(system, watch)`` auto-resolves to this callable
...
async def watch_system_impl(
    ...
    **_kwargs,
    ...
    """Watch a system-scoped run in ``group_id``."""
