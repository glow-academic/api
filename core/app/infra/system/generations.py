"""System generations logic — list generation groups for system.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, permissions)
  2. search_groups — core MV search with artifact_type="system" filter
"""
...
async def generations_system_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """List system generation groups — big-cache wrapped (L3)."""
    return await get_or_build(
        redis=redis,
        key=big_cache_key("system/generations", {
        ...
        response_model=GenerationsSystemApiResponse,
        builder=lambda: _generations_system_build(
...
async def _generations_system_build(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generations"):
        raise HTTPException(status_code=403, detail="You don't have permission to view system generations.")
    ...
    return GenerationsSystemApiResponse(
