"""Reports group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating reports groups.
...
class GroupReportsApiRequest(BaseModel):
    """Request model for reports group endpoint."""
...
class GroupReportsApiResponse(BaseModel):
    """Response model for reports group endpoint."""
...
async def group_reports_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupReportsApiRequest | None = None,
    ...
    Accepts either a GroupReportsApiRequest (HTTP/WS) or kwargs (internal).
    """
    ...
    return GroupReportsApiResponse(
        group_id=resolved_group_id,
