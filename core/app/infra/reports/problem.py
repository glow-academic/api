"""Reports problem logic — per-artifact diagnostic entry point.

Creates a problem entry scoped to the reports artifact type.
...
from app.infra.reports.types import ProblemReportsApiResponse
...
PROBLEM_TAGS = ["problems", "views", "reports"]
VALID_PROBLEM_TYPES = ("feature", "bug", "question", "other")


async def problem_reports_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """Create a problem entry for the reports artifact.

    Flow:
      1. Validate type and message
      2. Resolve profile identity context
      3. Permission check -- reports:problem
    ...
    if not has_permission(identity.role_permissions, ARTIFACT_TYPE, "problem"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to report reports problems.",
        )
    ...
    return ProblemReportsApiResponse(
