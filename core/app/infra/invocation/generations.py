"""Invocation generations logic — list generation groups for invocations.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, permissions)
  2. search_groups — core MV search with artifact_type="invocation" filter
"""
...
async def generations_invocation_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """List invocation generation groups.

    Flow:
      1. resolve_profile_identity_context -> role, permissions
      2. Permission check -- invocation:generations
      3. search_groups with artifact_type="invocation"
    """
    ...
    if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generations"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to view invocation generations.",
        )
    ...
    return GenerationsInvocationApiResponse(
