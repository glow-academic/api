"""Invocation group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating invocation groups.
...
class GroupInvocationApiRequest(BaseModel):
    """Request model for invocation group endpoint."""
...
class GroupInvocationApiResponse(BaseModel):
    """Response model for invocation group endpoint."""
...
async def group_invocation_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupInvocationApiRequest | None = None,
    group_id: UUID | None = None,
    name: str | None = None,
    window_seconds: int = DEFAULT_WINDOW_SECONDS,
) -> GroupInvocationApiResponse:
    """Resolve or create an invocation group with optional naming.
    ...
    Accepts either a GroupInvocationApiRequest (HTTP/WS) or kwargs (internal).
    """
    ...
    return GroupInvocationApiResponse(
        group_id=resolved_group_id,
