"""Invocation problem logic — per-artifact diagnostic entry point.

Creates a problem entry scoped to the invocation artifact type.
...
from app.infra.invocation.types import ProblemInvocationApiResponse
...
PROBLEM_TAGS = ["problems", "views", "invocations"]
VALID_PROBLEM_TYPES = ("feature", "bug", "question", "other")


async def problem_invocation_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """Create a problem entry for the invocation artifact.

    Flow:
      1. Validate type and message
      2. Resolve profile identity context
      3. Permission check -- invocation:problem
    ...
    if not has_permission(identity.role_permissions, ARTIFACT_TYPE, "problem"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to report invocation problems.",
        )
    ...
    return ProblemInvocationApiResponse(
