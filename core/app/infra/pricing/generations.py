"""Pricing generations logic — list generation groups for pricing.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, permissions)
  2. search_groups — core MV search with artifact_type="pricing" filter
"""
...
async def generations_pricing_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """List pricing generation groups.

    Flow:
      1. resolve_profile_identity_context -> role, permissions
      2. Permission check — pricing:generations
      3. search_groups with artifact_type="pricing"
    """
    ...
    if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generations"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to view pricing generations.",
        )
    ...
    return GenerationsPricingApiResponse(
