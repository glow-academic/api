"""Pricing group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating pricing groups.
...
class GroupPricingApiRequest(BaseModel):
    """Request model for pricing group endpoint."""
...
class GroupPricingApiResponse(BaseModel):
    """Response model for pricing group endpoint."""
...
async def group_pricing_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupPricingApiRequest | None = None,
    group_id: UUID | None = None,
    name: str | None = None,
    window_seconds: int = DEFAULT_WINDOW_SECONDS,
) -> GroupPricingApiResponse:
    """Resolve or create a pricing group with optional naming.
    ...
    Accepts either a GroupPricingApiRequest (HTTP/WS) or kwargs (internal).
    """
    ...
    return GroupPricingApiResponse(
        group_id=resolved_group_id,
