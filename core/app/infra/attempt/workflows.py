...
async def attempt_start_impl(
    ...
    *,
    ...
    # --- Resolve user identity + simulation context for ledger ---
    _ledger_email: str | None = None
    _ledger_name: str | None = None
    _ledger_role: str | None = None
    ...
    try:
        ...
        if _identity_for_ledger:
            ...
            _ledger_role = _identity_for_ledger.role_name
    except Exception:
    ...
    try:
        ...
        if payload.home_id:
            async with pool.acquire() as conn:
                _homes = await get_homes(conn, home_ids=[payload.home_id], redis=redis or Redis(), bypass_cache=True)
                if _homes:
                    _sim_ids = _homes[0].simulation_ids or []
                    if _sim_ids:
                        _sims = await _get_sims(conn, _sim_ids[:1], redis or Redis(), bypass_cache=True)
                        if _sims:
                            _sim_id = str(_sim_ids[0])
                            _sim_name = _sims[0].name

                        # Get passing threshold: rubric → points (type=total/pass) → compute_pass_pct
                        rubric_ids, _ = await search_rubrics(conn, simulation_ids=_sim_ids[:1], limit_count=1)
                        if rubric_ids:
                            rubrics = await get_rubrics(conn, rubric_ids, points=True)
                            if rubrics and rubrics[0].point_ids:
                                points = await get_points(conn, rubrics[0].point_ids, redis or Redis(), bypass_cache=True)
                                ...
                                if pct is not None:
                                    _pass_pct = round(pct / 100.0, 2)  # normalize to 0-1
    except Exception:
        pass  # Non-fatal — ledger works without simulation context
    ...
    try:
        ...
        sim_desc = None
        if simulation_ids:
            simulations = await get_simulations(pool,
                simulation_ids[:1],
                redis or Redis(),
                bypass_cache=True,
            )
