    ...
    *,
    ...
    with timed("db"):
     async with pool.acquire() as conn:
        ...
        # Department scope (mirrors the READ path, ``get_attempt_internal``
        # / ``enforce_attempt_media_access`` — issue #148): an instructional
        # actor may archive a cross-role attempt ONLY when its owner shares one
        # of their departments (or is global/roleless). Without it the write
        # gate was strictly wider than the read gate — a Dept-A instructor
        # could archive a Dept-B attempt they cannot even view. Resolved per
        # attempt; self / super-admin short-circuit inside the resolver so they
        # pay no extra query and keep self / global reach.
        scoped_attempts = []
        if requester is not None and requester_profiles_id is not None:
            for a in attempts:
                department_in_scope = True
                if (
                    a.profile_id is not None
                    and a.profile_id != requester_profiles_id
                ):
                    department_in_scope = await is_profile_in_department_scope(
                        pool, requester, a.profile_id
                    )
                if check_attempt_access(
                    a.profile_id,
