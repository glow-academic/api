"""Chat drafts list/search — composable infra architecture.

Composes existing black-box tools — no inline SQL:
  1. resolve_profile_identity_context — caller's profiles_id
  2. search_chat_drafts — declarative filters (name ILIKE, date window,
...
Single function powers both:
  ...
  - FE ``POST /chat/drafts`` HTTP route

Both consumers receive the same ``GetChatDraftsApiResponse`` shape; the
...
async def list_chat_drafts_impl(
    ...
    **_kwargs,
    ...
    """List/search chat drafts owned by the current profile."""
    with timed("profile"):
    ...
    return GetChatDraftsApiResponse(entries=drafts)
