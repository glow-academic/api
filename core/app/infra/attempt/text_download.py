"""Attempt text download logic — composable infra architecture.
...
async def text_download_attempt_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """Resolve a text entry to its file on disk.
    ...
    if not has_permission(profile.role_permissions, "attempt", "text_download"):
        raise HTTPException(
            status_code=403,
    ...
    return TextDownloadAttemptApiResult(
        upload_id=upload.id,
        file_path=file_path,
