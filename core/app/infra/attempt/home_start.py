"""Home start logic — create attempt from home entry.
...
Flow:
  ...
  2. Ledger gate check
  3. Resolve home entry -> simulation, chat entries, persona
  4. Create attempt (run -> call -> attempt -> attempt_home)
  5. Find first chat_id from home_chats
  6. Refresh MVs
...
from app.infra.attempt.practice_start import AttemptStartResponse
...
class HomeStartRequest(BaseModel):
    home_id: UUID


async def home_start_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    session_id: UUID,
    request: HomeStartRequest,
) -> AttemptStartResponse:
    """Create attempt from home entry. Returns attempt_id + first chat_id."""
    ...
    async with pool.acquire() as conn:
        entries = await get_homes(conn, [request.home_id])
    if not entries:
    ...
    # -- Step 3: Resolve home -> persona, chats, simulation --------------------

    profile_ids = parent_entry.profile_ids or []
    if not profile_ids:
        raise HTTPException(status_code=400, detail="No profile personas found in home.")
    ...
    async with pool.acquire() as conn:
        chat_entries = await search_home_chats(
            conn, home_ids=[request.home_id], limit=1000, bypass_mv=True,
        )
    if not chat_entries:
        raise HTTPException(status_code=400, detail="Home has no chat entries.")
