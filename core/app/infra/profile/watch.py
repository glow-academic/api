"""Profile watch — one-shot block-until-done for the tool layer.

The HTTP route ``GET /profile/watch`` stays as live SSE for the browser.
...
convention so ``(profile, watch)`` auto-resolves to this callable
...
async def watch_profile_impl(
    ...
    **_kwargs,
    ...
    """Watch a profile-scoped run in ``group_id``."""
