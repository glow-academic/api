...
async def generate_profile_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """Profile generation using deterministic infra functions.
    ...
    if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generate"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to generate profiles.",
        )
    ...
    try:
        ...
        logger.info(
            f"GENERATE_PROFILE: prepared run_id={prepared.run_id}, "
        ...
        # Step 5: Refresh all MVs (profile + shared infra) via canonical refresh
