"""Profile drafts list/search — composable infra architecture.

Composes existing black-box tools — no inline SQL:
  1. resolve_profile_identity_context — caller's profiles_id
  2. search_profile_drafts — declarative filters (name ILIKE, date window,
...
Single function powers both:
  ...
  - FE ``POST /profile/drafts`` HTTP route

Both consumers receive the same ``GetProfileDraftsApiResponse`` shape; the
...
async def list_profile_drafts_impl(
    ...
    **_kwargs,
    ...
    return GetProfileDraftsApiResponse(entries=drafts)
