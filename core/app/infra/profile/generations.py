"""Profile generations logic — list generation groups for profiles.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, permissions)
  2. search_groups — core MV search with artifact_type="profile" filter
"""
...
async def generations_profile_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """List profile generation groups — big-cache wrapped (L3)."""
    return await get_or_build(
        redis=redis,
        key=big_cache_key("profile/generations", {
        ...
        response_model=GenerationsProfileApiResponse,
        builder=lambda: _generations_profile_build(
...
async def _generations_profile_build(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """List profile generation groups.

    Flow:
      1. resolve_profile_identity_context → role, permissions
      2. Permission check — profile:generations
      3. search_groups with artifact_type="profile"
    """
    ...
    if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generations"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to view profile generations.",
        )
    ...
    return GenerationsProfileApiResponse(
