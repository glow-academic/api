"""Profile group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating profile groups.
...
class GroupProfileApiRequest(BaseModel):
    """Request model for profile group endpoint."""
...
class GroupProfileApiResponse(BaseModel):
    """Response model for profile group endpoint."""
...
async def group_profile_impl(
    ...
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupProfileApiRequest | None = None,
    ...
    Accepts either a GroupProfileApiRequest (HTTP/WS) or kwargs (internal).
    """
    ...
    return GroupProfileApiResponse(
        group_id=resolved_group_id,
