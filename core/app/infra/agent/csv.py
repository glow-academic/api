"""Agent CSV parse — accepts raw CSV bytes, auto-maps columns to import fields,
returns mapped items for client-side preview before bulk create.

Routes/agent/csv.py is a thin HTTP adapter over parse_agent_csv_impl.
"""
...
class ParseAgentCsvApiResponse(BaseModel):
...
def _row_to_item(row: list[str], field_map: dict[int, dict[str, Any]]) -> CreateAgentItem:
    kwargs: dict[str, Any] = {}
...
async def parse_agent_csv_impl(
    pool: asyncpg.Pool,
    *,
    session_id: UUID,
    ...
    **_kwargs,
) -> ParseAgentCsvApiResponse:
    """Parse a CSV file and return mapped agent items for preview.
    ...
    if accept is not None and idempotency_key is not None:
        ...
        return ParseAgentCsvApiResponse(
    ...
    if not field_map:
        labels = ", ".join(f["label"] for f in AGENT_IMPORT_FIELDS)
        raise CsvParseError(
    ...
    if soft and call_id is not None:
        async with pool.acquire() as conn:
            await create_soft_call(
                conn, redis, call_id=call_id, artifact="agent",
