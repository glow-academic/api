"""Agent generate logic — deterministic infra architecture."""
...
async def generate_agent_impl(
    ...
    *,
    ...
    """Agent generation using deterministic infra functions."""
    ...
    if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generate"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to generate agents.",
        )

    # Rate limit (RL-A / RL1/RL2) — agent/generate was the 4th LLM generation
    # entry point #379 missed, leaving the request_limit bypassable via the
    # agent artifact. Enforce here too (mirrors attempt/test/system generate),
    ...
    try:
        ...
        logger.info(
            f"GENERATE_AGENT: prepared run_id={prepared.run_id}, "
        ...
        logger.exception(f"Agent generation failed: {exc}")
        raise
