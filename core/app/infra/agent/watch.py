"""Agent watch — one-shot block-until-done for the tool layer.

The HTTP route ``GET /agent/watch`` stays as live SSE for the browser.
...
convention so ``(agent, watch)`` auto-resolves to this callable
...
async def watch_agent_impl(
    ...
    **_kwargs,
    ...
    """Watch a agent-scoped run in ``group_id``."""
