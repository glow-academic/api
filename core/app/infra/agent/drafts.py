"""Agent drafts list/search — composable infra architecture.

Composes existing black-box tools — no inline SQL:
  1. resolve_profile_identity_context — caller's profiles_id
  2. search_agent_drafts — declarative filters (name ILIKE, date window,
...
Single function powers both:
  ...
  - FE ``POST /agent/drafts`` HTTP route

Both consumers receive the same ``GetAgentDraftsApiResponse`` shape; the
...
async def list_agent_drafts_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """List/search agent drafts owned by the current profile."""
    with timed("profile"):
    ...
    return GetAgentDraftsApiResponse(entries=drafts)
