    ...
    *,
    ...
    if accept is not None and idempotency_key is not None:
        ...
        if entry is None or entry.status != "pending" or entry.operation != "duplicate":
            raise HTTPException(
                status_code=404,
                detail="No pending agent duplicate for this call.",
            )
        ...
        return DuplicateAgentApiResponse(
            success=True,
            agent_id=target_id,
            message="Agent duplicate accepted" if accept else "Agent duplicate rejected",
    ...
    # departments, else a Dept-A actor could clone a Dept-B agent
    # they cannot even view (mirrors ``scenario.duplicate``).
    if not compute_can_duplicate(
        role_level=profile.role_level,
        role_permissions=profile.role_permissions,
        agent_department_ids=original.department_ids,
