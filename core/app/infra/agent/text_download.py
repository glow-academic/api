"""Agent text download logic — composable infra architecture.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, departments)
  2. has_permission — permission check for agent:text_download
...
from app.infra.agent.types import TextDownloadAgentApiResult
...
async def text_download_agent_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    if not has_permission(profile.role_permissions, "agent", "text_download"):
        raise HTTPException(
            status_code=403,
    ...
    return TextDownloadAgentApiResult(
        upload_id=upload.id,
        file_path=file_path,
