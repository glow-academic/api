"""Agent group — thin wrapper over shared resolve_group_impl.
...
class GroupAgentApiRequest(GroupResolveRequest):
    """Request body for POST /agent/group."""


class GroupAgentApiResponse(GroupResolveResponse):
    """Response body for POST /agent/group."""
...
async def group_agent_impl(
    ...
    **kwargs,
    ...
    """Resolve/create an agent group; see resolve_group_impl for full semantics."""
    result = await resolve_group_impl(
        pool, redis, artifact_type=ARTIFACT_TYPE, **kwargs,
    )
    return GroupAgentApiResponse.model_validate(result.model_dump())
