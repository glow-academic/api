"""Chat group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating chat groups.
...
class GroupChatApiRequest(BaseModel):
    """Request model for chat group endpoint."""
...
class GroupChatApiResponse(BaseModel):
    """Response model for chat group endpoint."""
...
async def group_chat_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupChatApiRequest | None = None,
    group_id: UUID | None = None,
    name: str | None = None,
    window_seconds: int = DEFAULT_WINDOW_SECONDS,
) -> GroupChatApiResponse:
    """Resolve or create a chat group with optional naming.
    ...
    Accepts either a GroupChatApiRequest (HTTP/WS) or kwargs (internal).
    """
