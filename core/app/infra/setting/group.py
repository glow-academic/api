"""Setting group — thin wrapper over shared resolve_group_impl.
...
class GroupSettingApiRequest(GroupResolveRequest):
    """Request body for POST /setting/group."""


class GroupSettingApiResponse(GroupResolveResponse):
    """Response body for POST /setting/group."""
...
async def group_setting_impl(
    ...
    **kwargs,
    ...
    """Resolve/create an setting group; see resolve_group_impl for full semantics."""
    result = await resolve_group_impl(
        pool, redis, artifact_type=ARTIFACT_TYPE, **kwargs,
    )
    return GroupSettingApiResponse.model_validate(result.model_dump())
