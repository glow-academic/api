"""Setting problem logic — per-artifact diagnostic entry point.

Creates a problem entry scoped to the setting artifact type.
...
from app.infra.setting.types import ProblemSettingApiResponse
...
PROBLEM_TAGS = ["problems", "views", "settings"]
VALID_PROBLEM_TYPES = ("feature", "bug", "question", "other")


async def problem_setting_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """Create a problem entry for the setting artifact.

    Flow:
      1. Validate type and message
      2. Resolve profile identity context
      3. Permission check — setting:problem
    ...
    if not has_permission(identity.role_permissions, ARTIFACT_TYPE, "problem"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to report setting problems.",
        )
    ...
    return ProblemSettingApiResponse(
