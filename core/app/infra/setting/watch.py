"""Setting watch — one-shot block-until-done for the tool layer.

The HTTP route ``GET /setting/watch`` stays as live SSE for the browser.
...
convention so ``(setting, watch)`` auto-resolves to this callable
...
async def watch_setting_impl(
    ...
    **_kwargs,
    ...
    """Watch a setting-scoped run in ``group_id``."""
