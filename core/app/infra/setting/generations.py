"""Setting generations logic — list generation groups for settings.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, permissions)
  2. search_groups — core MV search with artifact_type="setting" filter
"""
...
async def generations_setting_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """List setting generation groups — big-cache wrapped (L3)."""
    return await get_or_build(
        redis=redis,
        key=big_cache_key("setting/generations", {
        ...
        tags=["context", "setting"],
        ttl_s=DEFAULT_BIG_CACHE_TTL_S,
        response_model=GenerationsSettingApiResponse,
        builder=lambda: _generations_setting_build(
...
async def _generations_setting_build(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """List setting generation groups.

    Flow:
      1. resolve_profile_identity_context → role, permissions
      2. Permission check — setting:generations
      3. search_groups with artifact_type="setting"
    """
    ...
    return GenerationsSettingApiResponse(
