"""Setting drafts list/search — composable infra architecture.

Composes existing black-box tools — no inline SQL:
  1. resolve_profile_identity_context — caller's profiles_id
  2. search_setting_drafts — declarative filters (name ILIKE, date window,
...
Single function powers both:
  ...
  - FE ``POST /setting/drafts`` HTTP route

Both consumers receive the same ``GetSettingDraftsApiResponse`` shape; the
...
async def list_setting_drafts_impl(
    ...
    **_kwargs,
    ...
    """List/search setting drafts owned by the current profile."""
    ...
    return GetSettingDraftsApiResponse(entries=drafts)
