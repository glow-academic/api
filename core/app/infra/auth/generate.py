"""Auth generate logic — deterministic infra architecture."""
...
async def generate_auth_impl(
    ...
    **_kwargs,
    ...
    """Auth generation using deterministic infra functions."""
    ...
    if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generate"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to generate auths.",
        )
    ...
    try:
        ...
        logger.info(
            f"GENERATE_AUTH: prepared run_id={prepared.run_id}, "
        ...
        logger.exception(f"Auth generation failed: {exc}")
        raise
