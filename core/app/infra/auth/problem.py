"""Auth problem logic — per-artifact diagnostic entry point.

Creates a problem entry scoped to the auth artifact type.
...
from app.infra.auth.types import ProblemAuthApiResponse
...
ARTIFACT_TYPE = "auth"
PROBLEM_TAGS = ["problems", "views", "auths"]
VALID_PROBLEM_TYPES = ("feature", "bug", "question", "other")


async def problem_auth_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """Create a problem entry for the auth artifact.

    Flow:
      1. Validate type and message
      2. Resolve profile identity context
      3. Permission check — auth:problem
    ...
    if not has_permission(identity.role_permissions, ARTIFACT_TYPE, "problem"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to report auth problems.",
        )
