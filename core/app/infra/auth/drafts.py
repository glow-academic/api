"""Auth drafts list/search — composable infra architecture.

Composes existing black-box tools — no inline SQL:
  1. resolve_profile_identity_context — caller's profiles_id
  2. search_auth_drafts — declarative filters (name ILIKE, date window,
...
Single function powers both:
  ...
  - FE ``POST /auth/drafts`` HTTP route

Both consumers receive the same ``GetAuthDraftsApiResponse`` shape; the
...
async def list_auth_drafts_impl(
    ...
    **_kwargs,
    ...
    """List/search auth drafts owned by the current profile."""
    with timed("profile"):
    ...
    return GetAuthDraftsApiResponse(entries=drafts)
