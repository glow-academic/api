"""Auth watch — one-shot block-until-done for the tool layer.

The HTTP route ``GET /auth/watch`` stays as live SSE for the browser.
...
convention so ``(auth, watch)`` auto-resolves to this callable
...
async def watch_auth_impl(
    ...
    **_kwargs,
    ...
    """Watch a auth-scoped run in ``group_id``."""
