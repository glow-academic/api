"""Auth generations logic — list generation groups for auths.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, permissions)
  2. search_groups — core MV search with artifact_type="auth" filter
"""
...
async def generations_auth_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """List auth generation groups — big-cache wrapped (L3)."""
    return await get_or_build(
        redis=redis,
        key=big_cache_key("auth/generations", {
        ...
        response_model=GenerationsAuthApiResponse,
        builder=lambda: _generations_auth_build(
...
async def _generations_auth_build(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """List auth generation groups.

    Flow:
      1. resolve_profile_identity_context → role, permissions
      2. Permission check — auth:generations
      3. search_groups with artifact_type="auth"
    """
    ...
    if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generations"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to view auth generations.",
        )
    ...
    return GenerationsAuthApiResponse(
