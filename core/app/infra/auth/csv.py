"""Auth CSV parse — accepts raw CSV bytes, auto-maps columns to import fields,
returns mapped items for client-side preview before bulk create.

Routes/auth/csv.py is a thin HTTP adapter over parse_auth_csv_impl.
"""
...
from app.infra.auth.search import AUTH_IMPORT_FIELDS
...
def _build_field_map(headers: list[str]) -> dict[int, dict[str, Any]]:
    ...
    for field in AUTH_IMPORT_FIELDS:
...
def _row_to_item(row: list[str], field_map: dict[int, dict[str, Any]]) -> CreateAuthItem:
    kwargs: dict[str, Any] = {}
    for col_idx, field_def in field_map.items():
        ...
        if field_type == "boolean":
            ...
            kwargs[key] = raw
    return CreateAuthItem(**kwargs)


async def parse_auth_csv_impl(
    pool: asyncpg.Pool,
    *,
    session_id: UUID,
    ...
    **_kwargs,
    ...
    """Parse a CSV file and return mapped auth items for preview.
    ...
    if accept is not None and idempotency_key is not None:
        async with pool.acquire() as conn:
            entry = await get_soft_call(conn, idempotency_key, redis, artifact="auth")
        ...
        async with pool.acquire() as conn:
            await create_soft_call(
                conn, redis, call_id=idempotency_key, artifact="auth",
                operation="csv", artifact_id=entry.artifact_id,
                status="accepted" if accept else "rejected",
            )
        return ParseAuthCsvApiResponse(
    ...
    if not field_map:
        labels = ", ".join(f["label"] for f in AUTH_IMPORT_FIELDS)
        raise CsvParseError(
    ...
    required_keys = {f["key"] for f in AUTH_IMPORT_FIELDS if f.get("required")}
    ...
    if soft and call_id is not None:
        async with pool.acquire() as conn:
            await create_soft_call(
                conn, redis, call_id=call_id, artifact="auth",
