"""Scenario file download logic — composable infra architecture.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, departments)
  2. has_permission — permission check for scenario:file_download
...
from app.infra.scenario.types import FileDownloadScenarioApiResult
from app.tools.entries.file_uploads.search import search_file_uploads
from app.tools.entries.uploads.get import get_upload


async def file_download_scenario_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    Flow:
      ...
      2. has_permission check (scenario:file_download)
    ...
    if not has_permission(profile.role_permissions, "scenario", "file_download"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to download scenario files.",
        )
    ...
    return FileDownloadScenarioApiResult(
        upload_id=upload.id,
        file_path=file_path,
