"""Scenario group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating scenario groups.
...
class GroupScenarioApiRequest(BaseModel):
    """Request model for scenario group endpoint."""
...
class GroupScenarioApiResponse(BaseModel):
    """Response model for scenario group endpoint."""
...
async def group_scenario_impl(
    ...
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupScenarioApiRequest | None = None,
    ...
    Accepts either a GroupScenarioApiRequest (HTTP/WS) or kwargs (internal).
    """
    ...
    return GroupScenarioApiResponse(
        group_id=resolved_group_id,
