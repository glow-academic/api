...
def compute_can_duplicate(
    role_level: int,
    role_permissions: list[tuple[str, str]],
    scenario_department_ids: list[str] | list[UUID] | None = None,
    user_department_ids: list[str] | list[UUID] | None = None,
) -> bool:
    ...
    - Non-top-level users must belong to ALL of the scenario's departments
      ...
      could clone a Dept-B scenario they cannot even edit, inheriting its
      department scope into the copy).
    """
    # Permission check
    if not has_permission(role_permissions, "scenario", "duplicate"):
        return False
...
def compute_can_assign_departments(
    ...
    """Whether the actor may write the *target* departments onto a scenario.
    ...
    create a scenario directly into Dept-B, or move/retag an editable Dept-A scenario
