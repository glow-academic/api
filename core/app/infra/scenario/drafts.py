...
Composes existing black-box tools — no inline SQL:
  ...
  2. search_scenario_drafts — declarative filters (name ILIKE, date window,
...
Single function powers both:
  ...
  - FE ``POST /scenario/drafts`` HTTP route

Both consumers receive the same ``GetScenarioDraftsApiResponse`` shape; the
...
async def list_scenario_drafts_impl(
    ...
    **_kwargs,
    ...
    return GetScenarioDraftsApiResponse(entries=drafts)
