"""Scenario generations logic — list generation groups for scenarios.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, permissions)
  2. search_groups — core MV search with artifact_type="scenario" filter
"""
...
async def generations_scenario_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """List scenario generation groups — big-cache wrapped (L3)."""
    return await get_or_build(
        redis=redis,
        key=big_cache_key("scenario/generations", {
        ...
        tags=["context", "scenario"],
        ttl_s=DEFAULT_BIG_CACHE_TTL_S,
        response_model=GenerationsScenarioApiResponse,
        builder=lambda: _generations_scenario_build(
...
async def _generations_scenario_build(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """List scenario generation groups.

    Flow:
      1. resolve_profile_identity_context → role, permissions
      2. Permission check — scenario:generations
      3. search_groups with artifact_type="scenario"
    """
    ...
    if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generations"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to view scenario generations.",
        )
    ...
    return GenerationsScenarioApiResponse(
