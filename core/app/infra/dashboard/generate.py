"""Dashboard generate logic — per-artifact generation entry point.

Fire-and-return: resolves identity, checks dashboard:generate permission,
...
async def generate_dashboard_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    Flow:
      ...
      2. Permission check -- dashboard:generate
    ...
    if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generate"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to generate dashboards.",
        )
