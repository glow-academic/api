"""Dashboard group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating dashboard groups.
...
class GroupDashboardApiRequest(BaseModel):
    """Request model for dashboard group endpoint."""
...
class GroupDashboardApiResponse(BaseModel):
    """Response model for dashboard group endpoint."""
...
async def group_dashboard_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupDashboardApiRequest | None = None,
    group_id: UUID | None = None,
    name: str | None = None,
    window_seconds: int = DEFAULT_WINDOW_SECONDS,
) -> GroupDashboardApiResponse:
    """Resolve or create a dashboard group with optional naming.
    ...
    Accepts either a GroupDashboardApiRequest (HTTP/WS) or kwargs (internal).
    """
    ...
    return GroupDashboardApiResponse(
        group_id=resolved_group_id,
