"""Record problem logic — per-artifact diagnostic entry point.

Creates a problem entry scoped to the record artifact type.
...
from app.infra.record.types import ProblemRecordApiResponse
...
PROBLEM_TAGS = ["problems", "views", "records"]
VALID_PROBLEM_TYPES = ("feature", "bug", "question", "other")


async def problem_record_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """Create a problem entry for the record artifact.

    Flow:
      1. Validate type and message
      2. Resolve profile identity context
      3. Permission check -- record:problem
    ...
    if not has_permission(identity.role_permissions, ARTIFACT_TYPE, "problem"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to report record problems.",
        )
    ...
    return ProblemRecordApiResponse(
