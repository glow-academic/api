"""Record group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating record groups.
...
class GroupRecordApiRequest(BaseModel):
    """Request model for record group endpoint."""
...
class GroupRecordApiResponse(BaseModel):
    """Response model for record group endpoint."""
...
async def group_record_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupRecordApiRequest | None = None,
    group_id: UUID | None = None,
    name: str | None = None,
    window_seconds: int = DEFAULT_WINDOW_SECONDS,
) -> GroupRecordApiResponse:
    """Resolve or create a record group with optional naming.
    ...
    Accepts either a GroupRecordApiRequest (HTTP/WS) or kwargs (internal).
    """
