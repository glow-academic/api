...
async def page_context_session_impl(
    ...
    **_kwargs,
    ...
    """session page context — big-cache wrapped."""
    return await get_or_build(
        redis=redis,
        key=big_cache_key("session/page_context", {
            "profile_id": str(profile_id),
            "entity_id": str(entity_id) if entity_id else None,
            "schema": schema,
        }),
        tags=["context", "session", "artifacts"],
        ttl_s=DEFAULT_BIG_CACHE_TTL_S,
        response_model=ComposedContextResponse,
        builder=lambda: _page_context_session_build(
            pool, redis,
...
async def _page_context_session_build(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    return ComposedContextResponse(
        ...
        entries=([sessions] if schema else None),
