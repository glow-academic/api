"""Session group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating session groups.
...
class GroupSessionApiRequest(BaseModel):
    """Request model for session group endpoint."""
...
class GroupSessionApiResponse(BaseModel):
    """Response model for session group endpoint."""
...
async def group_session_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupSessionApiRequest | None = None,
    group_id: UUID | None = None,
    name: str | None = None,
    window_seconds: int = DEFAULT_WINDOW_SECONDS,
) -> GroupSessionApiResponse:
    """Resolve or create a session group with optional naming.
    ...
    Accepts either a GroupSessionApiRequest (HTTP/WS) or kwargs (internal).
    """
    ...
    return GroupSessionApiResponse(
        group_id=resolved_group_id,
