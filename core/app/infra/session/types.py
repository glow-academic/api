...
class GenerationsSessionApiRequest(BaseModel):
    """Request model for session generations endpoint."""
...
class GenerationsSessionListItem(BaseModel):
    """Single generation group in the session generations response."""
...
class GenerationsSessionApiResponse(BaseModel):
    """Response model for session generations endpoint."""

    actor_name: str | None = Field(None, description="Display name of the current actor")
    items: list[GenerationsSessionListItem] = Field(default_factory=list, description="Generation groups")
...
class ProblemSessionApiRequest(BaseModel):
    """Request model for session problem endpoint."""

    type: str = Field(..., description="Problem type: feature, bug, question, other")
    message: str = Field(..., description="Problem description (max 1000 chars)")


class ProblemSessionApiResponse(BaseModel):
    """Response model for session problem endpoint."""
