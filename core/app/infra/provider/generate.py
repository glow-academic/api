...
async def generate_provider_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """Provider generation using deterministic infra functions.
    ...
    with timed("permissions"):
        if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generate"):
            raise HTTPException(
                status_code=403,
                detail="You don't have permission to generate providers.",
            )
    ...
    # (+agent); provider/generate is an LLM-cost entry point and was
    # bypassable. Enforce here too, before any provider call. No-op for
    # roles without a quota.
    ...
    try:
        ...
        logger.info(
            f"GENERATE_PROVIDER: prepared run_id={prepared.run_id}, "
        ...
        with timed("execute"):
            run_result = await run_generation_with_refresh(
                ...
                tool_soft=tool_soft,
                artifact_type=ARTIFACT_TYPE,
                refresh_fn=refresh_provider_impl,
                profile_id=profile_id,
                session_id=session_id,
                group_id=group_id,
