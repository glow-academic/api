"""Provider group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating provider groups.
...
class GroupProviderApiRequest(BaseModel):
    """Request model for provider group endpoint."""
...
class GroupProviderApiResponse(BaseModel):
    """Response model for provider group endpoint."""
...
async def group_provider_impl(
    ...
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupProviderApiRequest | None = None,
    ...
    Accepts either a GroupProviderApiRequest (HTTP/WS) or kwargs (internal).
    """
    ...
    return GroupProviderApiResponse(
        group_id=resolved_group_id,
