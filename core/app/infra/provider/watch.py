"""Provider watch — one-shot block-until-done for the tool layer.

The HTTP route ``GET /provider/watch`` stays as live SSE for the browser.
...
convention so ``(provider, watch)`` auto-resolves to this callable
...
async def watch_provider_impl(
    ...
    **_kwargs,
    ...
    """Watch a provider-scoped run in ``group_id``."""
