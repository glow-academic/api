"""Provider file download logic — composable infra architecture.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, departments)
  2. has_permission — permission check for provider:file_download
...
from app.infra.provider.types import FileDownloadProviderApiResult
...
async def file_download_provider_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    with timed("permissions"):
        if not has_permission(profile.role_permissions, "provider", "file_download"):
            raise HTTPException(
                status_code=403,
                detail="You don't have permission to download provider files.",
            )
    ...
    return FileDownloadProviderApiResult(
