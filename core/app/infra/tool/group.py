"""Tool group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating tool groups.
...
class GroupToolApiRequest(BaseModel):
    """Request model for tool group endpoint."""
...
class GroupToolApiResponse(BaseModel):
    """Response model for tool group endpoint."""
...
async def group_tool_impl(
    ...
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupToolApiRequest | None = None,
    ...
    Accepts either a GroupToolApiRequest (HTTP/WS) or kwargs (internal).
    """
