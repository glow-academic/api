"""Tool drafts list/search — composable infra architecture.

Composes existing black-box tools — no inline SQL:
  1. resolve_profile_identity_context — caller's profiles_id
  2. search_tool_drafts — declarative filters (name ILIKE, date window,
...
Single function powers both:
  ...
  - FE ``POST /tool/drafts`` HTTP route

Both consumers receive the same ``GetToolDraftsApiResponse`` shape; the
...
async def list_tool_drafts_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    return GetToolDraftsApiResponse(entries=drafts)
