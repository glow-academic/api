"""Tool CSV parse — accepts raw CSV bytes, auto-maps columns to import fields,
returns mapped items for client-side preview before bulk create.

Routes/tool/csv.py is a thin HTTP adapter over parse_tool_csv_impl.
"""
...
from app.infra.tool.types import CreateToolItem
...
async def parse_tool_csv_impl(
    ...
    **_kwargs,
    ...
    """Parse a CSV file and return mapped tool items for preview.
    ...
    if accept is not None and idempotency_key is not None:
        async with pool.acquire() as conn:
            entry = await get_soft_call(conn, idempotency_key, redis, artifact="tool")
        ...
        async with pool.acquire() as conn:
            await create_soft_call(
                conn, redis, call_id=idempotency_key, artifact="tool",
    ...
    if not field_map:
        labels = ", ".join(f["label"] for f in TOOL_IMPORT_FIELDS)
        raise CsvParseError(
    ...
    if soft and call_id is not None:
        async with pool.acquire() as conn:
            await create_soft_call(
                conn, redis, call_id=call_id, artifact="tool",
