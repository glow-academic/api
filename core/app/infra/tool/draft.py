...
async def _maybe_auto_accept_tool_draft(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    draft_id: UUID,
    ...
    """Auto-accept the tool draft when no pending fields remain."""
    ...
    async with pool.acquire() as conn:
        drafts = await get_tool_drafts(conn, [draft_id], active=None)
    if not drafts:
        return False
    draft = drafts[0]
    pending_lists = [
        ...
        getattr(draft, "pending_arg_ids", None),
        getattr(draft, "pending_arg_position_ids", None),
        getattr(draft, "pending_args_output_ids", None),
        getattr(draft, "pending_permission_ids", None),
    ]
    ...
    **kwargs: Any,
    ...
    if accept is not None and idempotency_key is not None:
        ...
        if entry is None or entry.status != "pending" or entry.operation != OPERATION:
            raise HTTPException(
                status_code=404,
                detail="No pending tool draft for this call.",
            )
        target_id = entry.artifact_id

        if accept:
            async with pool.acquire() as conn:
                drafts = await get_tool_drafts(conn, [target_id], active=None)
                async with conn.transaction():
                    if drafts:
                        draft = drafts[0]
                        await create_tool_draft(
    ...
    if not soft:
        await _maybe_auto_accept_tool_draft(
            pool, redis,
            draft_id=result.id,
            session_id=session_id,
            profile_ids=[profile.profiles_id],
