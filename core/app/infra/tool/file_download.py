"""Tool file download logic — composable infra architecture.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, departments)
  2. has_permission — permission check for tool:file_download
...
from app.infra.tool.types import FileDownloadToolApiResult
from app.infra.profile_identity_context import resolve_profile_identity_context
from app.tools.entries.files.search import search_files


async def file_download_tool_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    if not has_permission(profile.role_permissions, "tool", "file_download"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to download tool files.",
        )
    ...
    return FileDownloadToolApiResult(
