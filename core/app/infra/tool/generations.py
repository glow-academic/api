"""Tool generations logic — list generation groups for tools.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, permissions)
  2. search_groups — core MV search with artifact_type="tool" filter
"""
...
async def generations_tool_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """List tool generation groups — big-cache wrapped (L3)."""
    return await get_or_build(
        redis=redis,
        key=big_cache_key("tool/generations", {
        ...
        response_model=GenerationsToolApiResponse,
        builder=lambda: _generations_tool_build(
...
async def _generations_tool_build(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """List tool generation groups.

    Flow:
      1. resolve_profile_identity_context → role, permissions
      2. Permission check — tool:generations
      3. search_groups with artifact_type="tool"
    """
    ...
    if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generations"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to view tool generations.",
        )
    ...
    return GenerationsToolApiResponse(
