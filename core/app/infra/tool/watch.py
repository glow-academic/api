"""Tool watch — one-shot block-until-done for the tool layer.

The HTTP route ``GET /tool/watch`` stays as live SSE for the browser.
...
convention so ``(tool, watch)`` auto-resolves to this callable
...
async def watch_tool_impl(
    ...
    **_kwargs,
    ...
    """Watch a tool-scoped run in ``group_id``."""
