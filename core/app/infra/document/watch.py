"""Document watch — one-shot block-until-done for the tool layer.

The HTTP route ``GET /document/watch`` stays as live SSE for the browser.
...
convention so ``(document, watch)`` auto-resolves to this callable
...
async def watch_document_impl(
    ...
    **_kwargs,
    ...
    """Watch a document-scoped run in ``group_id``."""
