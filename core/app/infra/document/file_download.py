"""Document file download logic — composable infra architecture.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, departments)
  2. has_permission — permission check for document:file_download
...
from app.infra.document.types import FileDownloadDocumentApiResult
...
async def file_download_document_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    Flow:
      ...
      2. has_permission check (document:file_download)
    ...
    if not has_permission(profile.role_permissions, "document", "file_download"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to download document files.",
        )
    ...
    return FileDownloadDocumentApiResult(
