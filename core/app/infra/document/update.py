...
async def update_document_impl(
    ...
    *,
    ...
    from app.infra.document.permissions import compute_can_assign_departments
    ...
    with timed("resolve_values"):
     async with pool.acquire() as conn:
        for idx, item in enumerate(items):
            ...
            # document in their own department must not move/retag it into a department
            # they don't belong to via body `department_ids`. compute_can_edit only scopes
            # the document's *existing* departments, not the target set being written.
            if item.department_ids is not None and not compute_can_assign_departments(
                ...
                raise HTTPException(
                    status_code=403,
                    detail=f"Item {idx}: You can only assign documents to your own departments.",
                )
            if item_errors:
                has_errors = True
