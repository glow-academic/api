...
def compute_can_delete(
    ...
    - Non-top-level users must belong to ALL of the document's departments
      ...
      document they cannot even edit).
    """
    ...
    # Permission check
    if not has_permission(role_permissions, "document", "delete"):
        return False
...
def compute_can_duplicate(
    role_level: int,
    role_permissions: list[tuple[str, str]],
    document_department_ids: list[str] | list[UUID] | None = None,
    user_department_ids: list[str] | list[UUID] | None = None,
) -> bool:
    ...
    - Non-top-level users must belong to ALL of the document's departments
      ...
      could clone a Dept-B document they cannot even edit, inheriting its
      department scope into the copy).
    """
    # Permission check
    if not has_permission(role_permissions, "document", "duplicate"):
        return False
...
def compute_can_assign_departments(
    ...
    """Whether the actor may write the *target* departments onto a document.
    ...
    create a document directly into Dept-B, or move/retag an editable Dept-A document
