"""Document group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating document groups.
...
class GroupDocumentApiRequest(BaseModel):
    """Request model for document group endpoint."""
...
class GroupDocumentApiResponse(BaseModel):
    """Response model for document group endpoint."""
...
async def group_document_impl(
    ...
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupDocumentApiRequest | None = None,
    ...
    Accepts either a GroupDocumentApiRequest (HTTP/WS) or kwargs (internal).
    """
