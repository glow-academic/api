...
async def generate_document_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """Document generation using deterministic infra functions.
    ...
    if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generate"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to generate documents.",
        )
    ...
    try:
        ...
        logger.info(
            f"GENERATE_DOCUMENT: prepared run_id={prepared.run_id}, "
        ...
        # Step 5: Refresh all MVs (document + shared infra) via canonical refresh.
