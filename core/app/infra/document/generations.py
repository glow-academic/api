"""Document generations logic — list generation groups for documents.

Composes existing black-box tools:
  1. resolve_profile_identity_context — profile (role, permissions)
  2. search_groups — core MV search with artifact_type="document" filter
"""
...
async def generations_document_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    **_kwargs,
    ...
    """List document generation groups — big-cache wrapped (L3)."""
    return await get_or_build(
        redis=redis,
        key=big_cache_key("document/generations", {
        ...
        builder=lambda: _generations_document_build(
...
async def _generations_document_build(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """List document generation groups.

    Flow:
      1. resolve_profile_identity_context -> role, permissions
      2. Permission check — document:generations
      3. search_groups with artifact_type="document"
    """
    ...
    with timed("permissions"):
        if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generations"):
            raise HTTPException(
                status_code=403,
                detail="You don't have permission to view document generations.",
            )
    ...
    return GenerationsDocumentApiResponse(
