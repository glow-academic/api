"""Document CSV parse — accepts raw CSV bytes, auto-maps columns to import fields,
returns mapped items for client-side preview before bulk create.

Routes/document/csv.py is a thin HTTP adapter over parse_document_csv_impl.
"""
...
from app.infra.document.types import CreateDocumentItem
...
async def parse_document_csv_impl(
    pool: asyncpg.Pool,
    *,
    session_id: UUID,
    ...
    **_kwargs,
    ...
    """Parse a CSV file and return mapped document items for preview.
    ...
    if accept is not None and idempotency_key is not None:
        async with pool.acquire() as conn:
            entry = await get_soft_call(conn, idempotency_key, redis, artifact="document")
        ...
        async with pool.acquire() as conn:
            await create_soft_call(
                conn, redis, call_id=idempotency_key, artifact="document",
    ...
    if not field_map:
        labels = ", ".join(f["label"] for f in DOCUMENT_IMPORT_FIELDS)
        raise CsvParseError(
    ...
    if soft and call_id is not None:
        async with pool.acquire() as conn:
            await create_soft_call(
                conn, redis, call_id=call_id, artifact="document",
