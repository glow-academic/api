"""Benchmark group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating benchmark groups.
...
class GroupBenchmarkApiRequest(BaseModel):
    """Request model for benchmark group endpoint."""
...
class GroupBenchmarkApiResponse(BaseModel):
    """Response model for benchmark group endpoint."""
...
async def group_benchmark_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupBenchmarkApiRequest | None = None,
    group_id: UUID | None = None,
    name: str | None = None,
    window_seconds: int = DEFAULT_WINDOW_SECONDS,
) -> GroupBenchmarkApiResponse:
    """Resolve or create a benchmark group with optional naming.
    ...
    Accepts either a GroupBenchmarkApiRequest (HTTP/WS) or kwargs (internal).
    """
    ...
    return GroupBenchmarkApiResponse(
        group_id=resolved_group_id,
