"""Benchmark generate logic — per-artifact generation entry point.

Fire-and-return: resolves identity, checks benchmark:generate permission,
...
async def generate_benchmark_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    ...
    """Trigger benchmark generation.

    Flow:
      1. resolve_profile_identity_context -> role, permissions
      2. Permission check -- benchmark:generate
    ...
    if not has_permission(profile.role_permissions, ARTIFACT_TYPE, "generate"):
        raise HTTPException(
            status_code=403,
            detail="You don't have permission to generate benchmarks.",
        )
