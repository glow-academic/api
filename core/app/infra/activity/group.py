"""Activity group logic — time-windowed artifact grouping.

Canonical infra function for resolving or creating activity groups.
...
class GroupActivityApiRequest(BaseModel):
    """Request model for activity group endpoint."""
...
class GroupActivityApiResponse(BaseModel):
    """Response model for activity group endpoint."""
...
async def group_activity_impl(
    pool: asyncpg.Pool,
    redis: Redis,
    *,
    profile_id: UUID,
    session_id: UUID,
    request: GroupActivityApiRequest | None = None,
    group_id: UUID | None = None,
    name: str | None = None,
    window_seconds: int = DEFAULT_WINDOW_SECONDS,
) -> GroupActivityApiResponse:
    """Resolve or create an activity group with optional naming.
    ...
    Accepts either a GroupActivityApiRequest (HTTP/WS) or kwargs (internal).
    """
    ...
    return GroupActivityApiResponse(
        group_id=resolved_group_id,
