"""Input: cohort.search"""

from typing import Any
from uuid import UUID

from pydantic import Field

from app.infra.cohort.search import search_cohort_impl
from app.infra.events.audit import run_artifact_operation_with_audit
from app.infra.globals import get_internal_sio, get_pool, get_redis_client, sio
from app.infra.identity.socket import resolve_socket_identity
from app.ws.pagination import MAX_WS_PAGE_SIZE, PaginatedWsSearch

internal_sio = get_internal_sio()


class CohortSearchPayload(PaginatedWsSearch):
    """Payload for cohort.search socket event."""

    search: str | None = Field(None)
    filter_profile_ids: list[UUID] | None = Field(None)
    filter_simulation_ids: list[UUID] | None = Field(None)
    filter_department_ids: list[UUID] | None = Field(None)
    profile_search: str | None = Field(None)
    simulation_search: str | None = Field(None)
    department_search: str | None = Field(None)
    flag_search: str | None = Field(None)
    page_size: int = Field(10, ge=1, le=MAX_WS_PAGE_SIZE)


@sio.on("cohort.search")  # type: ignore
async def cohort_search(sid: str, data: dict[str, Any]) -> None:
    identity = await resolve_socket_identity(sid)
    if not identity:
        return

    try:
        payload = CohortSearchPayload(**data)
    except Exception as e:
        await internal_sio.emit("cohort.search.failed", {
            "sid": sid,
            "rooms": [sid],
            "message": str(e),
            "error_type": "validation",
        })
        return

    pool = get_pool()
    redis = get_redis_client()

    await run_artifact_operation_with_audit(
        pool,
        redis,
        artifact="cohort",
        operation="search",
        profile_id=identity.profile_id,
        sid=sid,
        runner=lambda: search_cohort_impl(
            pool,
            redis,
            profile_id=identity.profile_id,
            search=payload.search,
            filter_profile_ids=payload.filter_profile_ids,
            filter_simulation_ids=payload.filter_simulation_ids,
            filter_department_ids=payload.filter_department_ids,
            profile_search=payload.profile_search,
            simulation_search=payload.simulation_search,
            department_search=payload.department_search,
            flag_search=payload.flag_search,
            page_size=payload.page_size,
            page_offset=payload.page_offset,
        ),
        arguments=payload.model_dump(mode="json"),
    )
