...
async def disconnect(sid: str) -> None:
    ...
    if profile_to_cleanup:
        # Remove just THIS sid from the profile's set. Returns True iff
        # this was the last socket for the profile, in which case we
        # also flip presence to inactive.
        was_last = await remove_socket_owner(profile_to_cleanup, sid)
        if was_last:
