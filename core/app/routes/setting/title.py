"""Setting title endpoint — composable infra architecture.

Thin route handler. Core logic lives in app.infra.setting.title (which
wraps the shared title_group_impl). Writes a new ``group_names_entry`` row
renaming the setting's associated group.
"""
...
from app.infra.setting.title import (
...
@router.post("/title", response_model=TitleSettingApiResponse)
async def title_setting(
    request: TitleSettingApiRequest,
    ...
    try:
        ...
        async def _runner() -> TitleSettingApiResponse:
            return await title_setting_impl(
                pool,
                redis,
        ...
        response_data = await run_artifact_operation_with_audit(
            ...
            response_model=TitleSettingApiResponse,
            runner=_runner,
