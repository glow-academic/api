...
async def update_setting(
    ...
    try:
        ...
        response_data = await run_artifact_operation_with_audit(
            ...
            # (delete/all-matching shape, ack shape, or explicit-settings
            # shape — all serialize cleanly). ``request.settings`` is
            # None under ``all=true`` and ack paths, so we can't grab
            # just that field. ``mode="json"`` so UUIDs/datetimes
            # serialize via Pydantic's JSON encoder.
