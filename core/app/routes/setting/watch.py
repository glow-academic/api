"""GET /setting/watch — live SSE endpoint (replaces /setting/stream).

GET + SSE is fixed at the HTTP boundary so browser ``EventSource`` works.
LLM tools dispatch to ``watch_setting_impl`` (one-shot) via INFRA_OPS, not
...
from app.infra.setting.watch import watch_setting_impl
...
async def setting_watch(
    ...
    run_id: UUID | None = Query(
        ...
        description=(
            ...
            "run_id matches are streamed. Omit to receive every setting event "
...
async def setting_watch_once(
    ...
    async def _runner() -> WatchApiResponse:
        return await watch_setting_impl(
            pool,
            redis,
