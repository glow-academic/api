"""Reports context endpoint — page bootstrap with docs + profile + permissions."""
...
from app.infra.reports.page_context import page_context_reports_impl

router = APIRouter()


@router.post("/context", response_model=ComposedContextResponse)
async def get_reports_context(
    body: DocsApiRequest,
    http_request: Request,
    response: Response,
) -> ComposedContextResponse:
    """Get page context for the reports artifact.
    ...
    async def _runner() -> ComposedContextResponse:
        return await page_context_reports_impl(
            pool,
            redis,
    ...
    response.headers["X-Cache-Tags"] = "reports"
    return result
