"""Parameter title endpoint — composable infra architecture.

Thin route handler. Core logic lives in app.infra.parameter.title (which
wraps the shared title_group_impl). Writes a new ``group_names_entry`` row
renaming the parameter's associated group.
"""
...
from app.infra.parameter.title import (
    TitleParameterApiRequest,
    TitleParameterApiResponse,
    title_parameter_impl,
)
...
@router.post("/title", response_model=TitleParameterApiResponse)
async def title_parameter(
    request: TitleParameterApiRequest,
    ...
    try:
        ...
        async def _runner() -> TitleParameterApiResponse:
            return await title_parameter_impl(
                pool,
                redis,
        ...
        response_data = await run_artifact_operation_with_audit(
            ...
            response_model=TitleParameterApiResponse,
            runner=_runner,
