"""GET /parameter/watch — live SSE endpoint (replaces /parameter/stream).

GET + SSE is fixed at the HTTP boundary so browser ``EventSource`` works.
LLM tools dispatch to ``watch_parameter_impl`` (one-shot) via INFRA_OPS, not
...
from app.infra.parameter.watch import watch_parameter_impl
...
async def parameter_watch(
    ...
    run_id: UUID | None = Query(
        ...
        description=(
            ...
            "run_id matches are streamed. Omit to receive every parameter event "
...
async def parameter_watch_once(
    ...
    async def _runner() -> WatchApiResponse:
        return await watch_parameter_impl(
            pool,
            redis,
