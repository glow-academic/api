"""Field title endpoint — composable infra architecture.

Thin route handler. Core logic lives in app.infra.field.title (which
wraps the shared title_group_impl). Writes a new ``group_names_entry`` row
renaming the field's associated group.
"""
...
from app.infra.field.title import (
...
@router.post("/title", response_model=TitleFieldApiResponse)
async def title_field(
    request: TitleFieldApiRequest,
    ...
    try:
        ...
        async def _runner() -> TitleFieldApiResponse:
            return await title_field_impl(
                pool,
                redis,
        ...
        response_data = await run_artifact_operation_with_audit(
            ...
            response_model=TitleFieldApiResponse,
            runner=_runner,
