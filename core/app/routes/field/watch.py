"""GET /field/watch — live SSE endpoint (replaces /field/stream).

GET + SSE is fixed at the HTTP boundary so browser ``EventSource`` works.
LLM tools dispatch to ``watch_field_impl`` (one-shot) via INFRA_OPS, not
...
async def field_watch(
    ...
    run_id: UUID | None = Query(
        ...
        description=(
            ...
            "run_id matches are streamed. Omit to receive every field event "
    ...
    return await stream_field_impl(
