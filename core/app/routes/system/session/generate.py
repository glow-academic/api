"""Session generate endpoint — HTTP adapter for per-artifact generation.

Thin route handler. Core logic lives in app.infra.session.generate.
...
from app.infra.session.generate import generate_session_impl
...
async def generate_session(
    ...
    """Trigger session generation. Returns immediately; progress via events."""
    try:
        ...
        async def _runner() -> ArtifactGenerateResponse:
            return await generate_session_impl(
                pool,
                redis,
