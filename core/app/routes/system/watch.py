"""GET /system/watch — live SSE endpoint (replaces /system/stream).

GET + SSE is fixed at the HTTP boundary so browser ``EventSource`` works.
LLM tools dispatch to ``watch_system_impl`` (one-shot) via INFRA_OPS, not
...
from app.infra.system.stream import stream_system_impl
from app.infra.system.watch import watch_system_impl
...
async def system_watch(
    ...
    run_id: UUID | None = Query(
        ...
        description=(
            ...
            "run_id matches are streamed. Omit to receive every system event "
    ...
    return await stream_system_impl(
...
async def system_watch_once(
    ...
    async def _runner() -> WatchApiResponse:
        return await watch_system_impl(
            pool,
            redis,
