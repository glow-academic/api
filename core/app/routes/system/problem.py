"""System problem endpoint — composable infra architecture.

Thin route handler. Core logic lives in app.infra.system.problem.
"""
...
@router.post("/problem", response_model=ProblemSystemApiResponse)
async def problem_system(
    request: ProblemSystemApiRequest,
    http_request: Request,
    response: Response,
) -> ProblemSystemApiResponse:
    """Report a system problem — composable infra architecture."""
    tags = ["system", "problems"]
    ...
    try:
        ...
        async def _runner() -> ProblemSystemApiResponse:
            return await problem_system_impl(
                pool, redis, profile_id=profile_id, session_id=session_id,
        ...
        handle_route_error(error=e, route_path=http_request.url.path, operation="problem_system",
