"""System title endpoint — composable infra architecture.

Thin route handler. Core logic lives in app.infra.system.title (which
wraps the shared title_group_impl). Writes a new ``group_names_entry`` row
renaming the system's associated group.
"""
...
from app.infra.system.title import (
...
@router.post("/title", response_model=TitleSystemApiResponse)
async def title_system(
    request: TitleSystemApiRequest,
    ...
    try:
        ...
        async def _runner() -> TitleSystemApiResponse:
        ...
        response_data = await run_artifact_operation_with_audit(
            ...
            response_model=TitleSystemApiResponse,
            runner=_runner,
