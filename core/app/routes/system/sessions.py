"""Top-level paginated sessions endpoint.

Previously at /system/activity/search; promoted to /system/sessions because the
endpoint is fundamentally "search sessions" — activity data travels alongside
as the canonical shape. Other consumers that need session lists call this.
...
@router.post("/sessions", response_model=ListActivityResponse)
async def search_sessions(
    ...
    try:
        ...
        async def _runner() -> ListActivityResponse:
            if not bypass_cache:
