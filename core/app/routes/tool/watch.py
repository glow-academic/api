"""GET /tool/watch — live SSE endpoint (replaces /tool/stream).

GET + SSE is fixed at the HTTP boundary so browser ``EventSource`` works.
LLM tools dispatch to ``watch_tool_impl`` (one-shot) via INFRA_OPS, not
...
from app.infra.tool.stream import stream_tool_impl
from app.infra.tool.watch import watch_tool_impl
...
async def tool_watch(
    ...
    run_id: UUID | None = Query(
        ...
        description=(
            ...
            "run_id matches are streamed. Omit to receive every tool event "
