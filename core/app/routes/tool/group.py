"""Tool group endpoint — thin HTTP adapter.

Core logic lives in app.infra.tool.group.
"""
...
@router.post("/group", response_model=GroupToolApiResponse)
async def group_tool(
    request: GroupToolApiRequest,
    ...
    try:
        ...
        async def _runner(group_id: UUID) -> GroupToolApiResponse:
            scoped_request = request.model_copy(update={'group_id': group_id})
            return await group_tool_impl(
                pool,
                redis,
        ...
        result = await run_artifact_operation_with_audit(
            ...
            response_model=GroupToolApiResponse,
            runner=_runner,
