"""Tool title endpoint — composable infra architecture.

Thin route handler. Core logic lives in app.infra.tool.title (which
wraps the shared title_group_impl). Writes a new ``group_names_entry`` row
renaming the tool's associated group.
"""
...
from app.infra.tool.title import (
    TitleToolApiRequest,
...
@router.post("/title", response_model=TitleToolApiResponse)
async def title_tool(
    request: TitleToolApiRequest,
    ...
    try:
        ...
        async def _runner() -> TitleToolApiResponse:
            return await title_tool_impl(
                pool,
                redis,
        ...
        response_data = await run_artifact_operation_with_audit(
            ...
            response_model=TitleToolApiResponse,
            runner=_runner,
