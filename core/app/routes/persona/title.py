"""Persona title endpoint — composable infra architecture.

Thin route handler. Core logic lives in app.infra.persona.title (which
wraps the shared title_group_impl). Writes a new ``group_names_entry`` row
renaming the persona's associated group.
"""
...
from app.infra.persona.title import (
...
@router.post("/title", response_model=TitlePersonaApiResponse)
async def title_persona(
    request: TitlePersonaApiRequest,
