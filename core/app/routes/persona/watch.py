"""GET /persona/watch — live SSE endpoint (replaces /persona/stream).

GET + SSE is fixed at the HTTP boundary so browser ``EventSource`` works.
LLM tools dispatch to ``watch_persona_impl`` (one-shot) via INFRA_OPS, not
...
from app.infra.persona.watch import watch_persona_impl
...
async def persona_watch(
    ...
    run_id: UUID | None = Query(
        ...
        description=(
            ...
            "run_id matches are streamed. Omit to receive every persona event "
...
async def persona_watch_once(
    ...
    async def _runner() -> WatchApiResponse:
        return await watch_persona_impl(
            pool,
            redis,
