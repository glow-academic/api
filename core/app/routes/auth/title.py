"""Auth title endpoint — composable infra architecture.

Thin route handler. Core logic lives in app.infra.auth.title (which
wraps the shared title_group_impl). Writes a new ``group_names_entry`` row
renaming the auth's associated group.
"""
...
from app.infra.auth.title import (
    TitleAuthApiRequest,
...
@router.post("/title", response_model=TitleAuthApiResponse)
async def title_auth(
    request: TitleAuthApiRequest,
    ...
    try:
        ...
        async def _runner() -> TitleAuthApiResponse:
            return await title_auth_impl(
                pool,
                redis,
        ...
        response_data = await run_artifact_operation_with_audit(
            ...
            response_model=TitleAuthApiResponse,
            runner=_runner,
