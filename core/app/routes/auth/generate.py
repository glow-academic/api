"""Auth generate endpoint — HTTP adapter for per-artifact generation.

Thin route handler. Core logic lives in app.infra.auth.generate.
...
async def generate_auth(
    ...
    try:
        ...
        async def _runner() -> ArtifactGenerateResponse:
            return await generate_auth_impl(
                pool,
                redis,
