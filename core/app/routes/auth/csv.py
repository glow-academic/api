"""Auth CSV parse endpoint — accepts a CSV file and returns mapped items for preview."""
...
@router.post("/csv", response_model=ParseAuthCsvApiResponse)
async def parse_auth_csv(
    file: UploadFile,
    ...
    try:
        ...
        if not field_map:
            raise HTTPException(
                ...
                f"Expected: {', '.join(f['label'] for f in AUTH_IMPORT_FIELDS)}",
            )
