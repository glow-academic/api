"""GET /auth/watch — live SSE endpoint (replaces /auth/stream).

GET + SSE is fixed at the HTTP boundary so browser ``EventSource`` works.
LLM tools dispatch to ``watch_auth_impl`` (one-shot) via INFRA_OPS, not
...
from app.infra.auth.watch import watch_auth_impl
...
async def auth_watch(
    ...
    run_id: UUID | None = Query(
        ...
        description=(
            ...
            "run_id matches are streamed. Omit to receive every auth event "
