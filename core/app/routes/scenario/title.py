"""Scenario title endpoint — composable infra architecture.

Thin route handler. Core logic lives in app.infra.scenario.title (which
wraps the shared title_group_impl). Writes a new ``group_names_entry`` row
renaming the scenario's associated group.
"""
...
from app.infra.scenario.title import (
...
@router.post("/title", response_model=TitleScenarioApiResponse)
async def title_scenario(
    request: TitleScenarioApiRequest,
    ...
    try:
        ...
        async def _runner() -> TitleScenarioApiResponse:
            return await title_scenario_impl(
                pool,
                redis,
        ...
        response_data = await run_artifact_operation_with_audit(
            ...
            response_model=TitleScenarioApiResponse,
            runner=_runner,
