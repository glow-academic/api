"""Simulation title endpoint — composable infra architecture.

Thin route handler. Core logic lives in app.infra.simulation.title (which
wraps the shared title_group_impl). Writes a new ``group_names_entry`` row
renaming the simulation's associated group.
"""
...
from app.infra.simulation.title import (
    TitleSimulationApiRequest,
    TitleSimulationApiResponse,
    title_simulation_impl,
)
...
@router.post("/title", response_model=TitleSimulationApiResponse)
async def title_simulation(
    request: TitleSimulationApiRequest,
    ...
    try:
        ...
        async def _runner() -> TitleSimulationApiResponse:
            return await title_simulation_impl(
                pool,
                redis,
        ...
        response_data = await run_artifact_operation_with_audit(
            ...
            response_model=TitleSimulationApiResponse,
            runner=_runner,
