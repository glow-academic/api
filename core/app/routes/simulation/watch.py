"""GET /simulation/watch — live SSE endpoint (replaces /simulation/stream).

GET + SSE is fixed at the HTTP boundary so browser ``EventSource`` works.
LLM tools dispatch to ``watch_simulation_impl`` (one-shot) via INFRA_OPS, not
...
from app.infra.simulation.stream import stream_simulation_impl
from app.infra.simulation.watch import watch_simulation_impl
...
async def simulation_watch(
    ...
    run_id: UUID | None = Query(
        ...
        description=(
            ...
            "run_id matches are streamed. Omit to receive every simulation event "
    ...
    return await stream_simulation_impl(
...
async def simulation_watch_once(
    ...
    async def _runner() -> WatchApiResponse:
        return await watch_simulation_impl(
            pool,
            redis,
