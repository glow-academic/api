"""GET /cohort/watch — live SSE endpoint (replaces /cohort/stream).

GET + SSE is fixed at the HTTP boundary so browser ``EventSource`` works.
LLM tools dispatch to ``watch_cohort_impl`` (one-shot) via INFRA_OPS, not
...
from app.infra.cohort.stream import stream_cohort_impl
from app.infra.cohort.watch import watch_cohort_impl
...
async def cohort_watch(
    ...
    run_id: UUID | None = Query(
        ...
        description=(
            ...
            "run_id matches are streamed. Omit to receive every cohort event "
    ...
    return await stream_cohort_impl(
...
async def cohort_watch_once(
    ...
    async def _runner() -> WatchApiResponse:
        return await watch_cohort_impl(
            pool,
            redis,
