"""Model title endpoint — composable infra architecture.

Thin route handler. Core logic lives in app.infra.model.title (which
wraps the shared title_group_impl). Writes a new ``group_names_entry`` row
renaming the model's associated group.
"""
...
from app.infra.model.title import (
...
@router.post("/title", response_model=TitleModelApiResponse)
async def title_model(
    request: TitleModelApiRequest,
    ...
    try:
        ...
        async def _runner() -> TitleModelApiResponse:
            return await title_model_impl(
                pool,
                redis,
        ...
        response_data = await run_artifact_operation_with_audit(
            ...
            response_model=TitleModelApiResponse,
            runner=_runner,
