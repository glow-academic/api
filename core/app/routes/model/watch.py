"""GET /model/watch — live SSE endpoint (replaces /model/stream).

GET + SSE is fixed at the HTTP boundary so browser ``EventSource`` works.
LLM tools dispatch to ``watch_model_impl`` (one-shot) via INFRA_OPS, not
...
from app.infra.model.stream import stream_model_impl
from app.infra.model.watch import watch_model_impl
...
async def model_watch(
    ...
    run_id: UUID | None = Query(
        ...
        description=(
            ...
            "run_id matches are streamed. Omit to receive every model event "
    ...
    return await stream_model_impl(
