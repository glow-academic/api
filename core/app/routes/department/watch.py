"""GET /department/watch — live SSE endpoint (replaces /department/stream).

GET + SSE is fixed at the HTTP boundary so browser ``EventSource`` works.
LLM tools dispatch to ``watch_department_impl`` (one-shot) via INFRA_OPS, not
...
from app.infra.department.stream import stream_department_impl
from app.infra.department.watch import watch_department_impl
...
async def department_watch(
    ...
    run_id: UUID | None = Query(
        ...
        description=(
            ...
            "run_id matches are streamed. Omit to receive every department event "
    ...
    return await stream_department_impl(
...
async def department_watch_once(
    ...
    async def _runner() -> WatchApiResponse:
        return await watch_department_impl(
            pool,
            redis,
