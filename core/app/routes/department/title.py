"""Department title endpoint — composable infra architecture.

Thin route handler. Core logic lives in app.infra.department.title (which
wraps the shared title_group_impl). Writes a new ``group_names_entry`` row
renaming the department's associated group.
"""
...
from app.infra.department.title import (
    TitleDepartmentApiRequest,
    TitleDepartmentApiResponse,
    title_department_impl,
)
...
@router.post("/title", response_model=TitleDepartmentApiResponse)
async def title_department(
    request: TitleDepartmentApiRequest,
    ...
    try:
        ...
        async def _runner() -> TitleDepartmentApiResponse:
            return await title_department_impl(
                pool,
                redis,
        ...
        response_data = await run_artifact_operation_with_audit(
            ...
            response_model=TitleDepartmentApiResponse,
            runner=_runner,
