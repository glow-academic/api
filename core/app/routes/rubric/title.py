"""Rubric title endpoint — composable infra architecture.

Thin route handler. Core logic lives in app.infra.rubric.title (which
wraps the shared title_group_impl). Writes a new ``group_names_entry`` row
renaming the rubric's associated group.
"""
...
from app.infra.rubric.title import (
...
@router.post("/title", response_model=TitleRubricApiResponse)
async def title_rubric(
    request: TitleRubricApiRequest,
    ...
    try:
        ...
        async def _runner() -> TitleRubricApiResponse:
            return await title_rubric_impl(
                pool,
                redis,
        ...
        response_data = await run_artifact_operation_with_audit(
            ...
            response_model=TitleRubricApiResponse,
            runner=_runner,
