"""Agent title endpoint — composable infra architecture.

Thin route handler. Core logic lives in app.infra.agent.title (which
wraps the shared title_group_impl). Writes a new ``group_names_entry`` row
renaming the agent's associated group.
"""
...
from app.infra.agent.title import (
...
@router.post("/title", response_model=TitleAgentApiResponse)
async def title_agent(
    request: TitleAgentApiRequest,
    ...
    try:
        ...
        async def _runner() -> TitleAgentApiResponse:
            return await title_agent_impl(
                pool,
                redis,
        ...
        response_data = await run_artifact_operation_with_audit(
            ...
            response_model=TitleAgentApiResponse,
            runner=_runner,
