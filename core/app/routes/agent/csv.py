"""Agent CSV parse endpoint — accepts a CSV file and returns mapped items for preview."""
...
@router.post("/csv", response_model=ParseAgentCsvApiResponse)
async def parse_agent_csv(
    file: UploadFile,
    ...
    try:
        ...
        if not field_map:
            raise HTTPException(
                ...
                f"Expected: {', '.join(f['label'] for f in AGENT_IMPORT_FIELDS)}",
            )
