"""GET /test/watch — live SSE endpoint (replaces /test/stream).

GET + SSE is fixed at the HTTP boundary so browser ``EventSource`` works.
LLM tools dispatch to ``watch_test_impl`` (one-shot) via INFRA_OPS, not
...
async def test_watch(
    ...
    run_id: UUID | None = Query(
        ...
        description=(
            ...
            "run_id matches are streamed. Omit to receive every test event "
