"""Test title endpoint — composable infra architecture.

Thin route handler. Core logic lives in app.infra.test.title (which
wraps the shared title_group_impl). Writes a new ``group_names_entry`` row
renaming the test's associated group.
"""
...
from app.infra.test.title import (
    TitleTestApiRequest,
...
@router.post("/title", response_model=TitleTestApiResponse)
async def title_test(
    request: TitleTestApiRequest,
    ...
    try:
        ...
        async def _runner() -> TitleTestApiResponse:
        ...
        response_data = await run_artifact_operation_with_audit(
            ...
            response_model=TitleTestApiResponse,
            runner=_runner,
