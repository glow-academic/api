"""Benchmark generate endpoint — HTTP adapter for per-artifact generation.

Thin route handler. Core logic lives in app.infra.benchmark.generate.
...
from app.infra.benchmark.generate import generate_benchmark_impl
...
async def generate_benchmark(
    ...
    """Trigger benchmark generation. Returns immediately; progress via events."""
    try:
        ...
        async def _runner() -> ArtifactGenerateResponse:
            return await generate_benchmark_impl(
                pool,
                redis,
