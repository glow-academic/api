"""Invocation context endpoint — page bootstrap with docs + profile + permissions."""
...
from app.infra.invocation.page_context import page_context_invocation_impl

router = APIRouter()


@router.post("/context", response_model=ComposedContextResponse)
async def get_invocation_context(
    body: DocsApiRequest,
    http_request: Request,
    response: Response,
) -> ComposedContextResponse:
    """Get page context for the invocation analytics.
    ...
    async def _runner() -> ComposedContextResponse:
        return await page_context_invocation_impl(
            pool,
            redis,
    ...
    response.headers["X-Cache-Tags"] = "invocations"
    return result
