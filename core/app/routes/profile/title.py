"""Profile title endpoint — composable infra architecture.

Thin route handler. Core logic lives in app.infra.profile.title (which
wraps the shared title_group_impl). Writes a new ``group_names_entry`` row
renaming the profile's associated group.
"""
...
from app.infra.profile.title import (
...
@router.post("/title", response_model=TitleProfileApiResponse)
async def title_profile(
    request: TitleProfileApiRequest,
    ...
    try:
        ...
        async def _runner() -> TitleProfileApiResponse:
            return await title_profile_impl(
                pool,
                redis,
