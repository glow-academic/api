"""GET /profile/watch — live SSE endpoint (replaces /profile/stream).

GET + SSE is fixed at the HTTP boundary so browser ``EventSource`` works.
LLM tools dispatch to ``watch_profile_impl`` (one-shot) via INFRA_OPS, not
...
from app.infra.profile.watch import watch_profile_impl
...
async def profile_watch(
    ...
    run_id: UUID | None = Query(
        ...
        description=(
            ...
            "run_id matches are streamed. Omit to receive every profile event "
...
async def profile_watch_once(
    ...
    async def _runner() -> WatchApiResponse:
        return await watch_profile_impl(
            pool,
            redis,
