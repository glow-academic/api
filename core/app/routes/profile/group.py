"""Profile group endpoint — thin HTTP adapter.

Core logic lives in app.infra.profile.group.
"""
...
@router.post("/group", response_model=GroupProfileApiResponse)
async def group_profile(
    request: GroupProfileApiRequest,
    ...
    try:
        ...
        async def _runner(group_id: UUID) -> GroupProfileApiResponse:
            scoped_request = request.model_copy(update={'group_id': group_id})
            return await group_profile_impl(
                pool,
                redis,
        ...
        result = await run_artifact_operation_with_audit(
            ...
            response_model=GroupProfileApiResponse,
            runner=_runner,
