"""GET /provider/watch — live SSE endpoint (replaces /provider/stream).

GET + SSE is fixed at the HTTP boundary so browser ``EventSource`` works.
LLM tools dispatch to ``watch_provider_impl`` (one-shot) via INFRA_OPS, not
...
from app.infra.provider.stream import stream_provider_impl
from app.infra.provider.watch import watch_provider_impl
...
async def provider_watch(
    ...
    run_id: UUID | None = Query(
        ...
        description=(
            ...
            "run_id matches are streamed. Omit to receive every provider event "
    ...
    return await stream_provider_impl(
...
async def provider_watch_once(
    ...
    async def _runner() -> WatchApiResponse:
        return await watch_provider_impl(
            pool,
            redis,
