"""Provider title endpoint — composable infra architecture.

Thin route handler. Core logic lives in app.infra.provider.title (which
wraps the shared title_group_impl). Writes a new ``group_names_entry`` row
renaming the provider's associated group.
"""
...
from app.infra.provider.title import (
...
@router.post("/title", response_model=TitleProviderApiResponse)
async def title_provider(
    request: TitleProviderApiRequest,
    ...
    try:
        ...
        async def _runner() -> TitleProviderApiResponse:
            return await title_provider_impl(
                pool,
                redis,
        ...
        response_data = await run_artifact_operation_with_audit(
            ...
            response_model=TitleProviderApiResponse,
            runner=_runner,
