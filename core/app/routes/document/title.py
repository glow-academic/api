"""Document title endpoint — composable infra architecture.

Thin route handler. Core logic lives in app.infra.document.title (which
wraps the shared title_group_impl). Writes a new ``group_names_entry`` row
renaming the document's associated group.
"""
...
from app.infra.document.title import (
...
@router.post("/title", response_model=TitleDocumentApiResponse)
async def title_document(
    request: TitleDocumentApiRequest,
    ...
    try:
        ...
        async def _runner() -> TitleDocumentApiResponse:
            return await title_document_impl(
                pool,
                redis,
        ...
        response_data = await run_artifact_operation_with_audit(
            ...
            response_model=TitleDocumentApiResponse,
            runner=_runner,
