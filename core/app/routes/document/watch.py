"""GET /document/watch — live SSE endpoint (replaces /document/stream).

GET + SSE is fixed at the HTTP boundary so browser ``EventSource`` works.
LLM tools dispatch to ``watch_document_impl`` (one-shot) via INFRA_OPS, not
...
from app.infra.document.stream import stream_document_impl
from app.infra.document.watch import watch_document_impl
...
async def document_watch(
    ...
    run_id: UUID | None = Query(
        ...
        description=(
            ...
            "run_id matches are streamed. Omit to receive every document event "
    ...
    return await stream_document_impl(
...
async def document_watch_once(
    ...
    async def _runner() -> WatchApiResponse:
        return await watch_document_impl(
            pool,
            redis,
